ad_proc iter_cari_rce_xml_tipo_1 {
    {
	-cod_batc      ""
        -id_utente     ""
	-file_name     ""
    ...
    @author          Luca Romitti (clonato da iter-cari-rce-tipo-1-procs.tcl)
    ...
    rom03            Se non trovo il relativo cod_strumento devo dare esito negativo con
    rom03            relativo messaggio di errore.
    rom03            Il controllo sul formato del deprimometro va fatto solo se valorizzato il campo.
    rom03            I casi in cui il campo e' obbligatorio e' gia' gestito con la modifica rom02.
    rom03            Inoltre Sandro ha chiesto di togliere il controllo sull'indirizzo
    rom03            del responsabile solo sulla Regione Marche.
    ...
    set csv_name [parameter::get_from_package_key -package_key iter -parameter tabella_caricamento_rcee_tipo_1]

    
    ns_log Notice "Inizio della proc iter_cari_rce_xml_tipo_1. id_utente = $id_utente, file_name = $file_name, nome_tabella = $nome_tabella,"
	...
	if {![string equal $nome_tabella ""]} {
	    set nome_tabella_anom [string map {XRCEE1CARI ANOM} $nome_tabella]
	    set nome_sequence $nome_tabella
	    append nome_sequence "_s"
	    ns_log notice "LUCAR TEST"
	...
	#se non mi viene passato il nome della tabella da analizzare significa che ho un nuovo caricamento
	if {[string equal $nome_tabella ""]} {
	    set file_inp_name  "Caricamento-rcee-xml-tipo-1-input"
	...
	set file_esi_name  "Caricamento-rcee-xml-tipo-1-esito"
	set file_esi_name  [iter_temp_file_name -permanenti $file_esi_name]
	
	set file_err_name  "Caricamento-rcee-xml-tipo-1-scartati"
	set file_err_name  [iter_temp_file_name -permanenti $file_err_name]
	
	set file_tot_name  "Caricamento-rcee-xml-tipo-1-file-xml"
	...
	if {[string equal $nome_tabella ""]} {
	    ...
	    set file_inp       "${permanenti_dir}/$file_inp_name.xml"
	    set file_table     "${permanenti_dir}/$file_table_name.sql"
	    set file_inp_url   "${permanenti_dir_url}/$file_inp_name.xml"
	...
	if {[string equal $nome_tabella ""]} {
	    ...
	    ns_log notice "test 1 file_inp $file_inp file_name $file_name file_inp_name $file_inp_name"
	    
#	    if {[catch {set file_inp_id [open $file_inp r]}]} {
#		iter_return_complaint "File xml di input non aperto: $file_inp"
	    #	    }
	    set         file_inp_id  [open $file_inp r]	    
	    # dichiaro di leggere in formato utf-8
	    fconfigure $file_inp_id -encoding utf-8
	    #set response [read $file_id]
	    set response [read $file_inp_id]
	    close      $file_inp_id
	    ns_log notice "test 2 file_inp_id $file_inp_id"
	...
	set ls_coimtabs [db_list_of_lists q "select nome_colonna
                                                      , denominazione
                                                      , tipo_dato
                                                      , dimensione
                                                      , obbligatorio
	...
	foreach record_coimtabs $ls_coimtabs {
	    lassign $record_coimtabs nome_colonna denominazione tipo_dato dimensione obbligatorio default_value range_value
	    # Popolo i campi della lista
	    lappend file_cols $nome_colonna
	    
	    #Memorizzo in un array tutti i dati relativi ad un singolo campo, necessari per la successiva analisi
	...
	set q_ins_tabs ""
		...
		#verifico se mi è stato restituito un errore:
		set root_id  [ah_xml_get_root_id $response];#equivale a caricamento_rcee
	    } {
		append err_log "L'intero lotto &egrave; stato scartato in quanto il tracciato non è nel formato/ordine corretto."
		set root_id ""
		#set file_inp_id ""
	    }
		...
		with_catch msg_err_curl {
		    #verifico se mi è stato restituito un errore:
		    set node_ids [ah_xml_get_node_id $root_id "rcee"]
		} {
		    append err_log "L'intero lotto &egrave; stato scartato in quanto il tracciato non è nel formato/ordine corretto."		
		    set node_ids ""
		}
	    }
	    ns_log notice "test 3 root_id $root_id response $response err_log $err_log"
	    
	    if {[string equal $err_log ""]} {
		ns_log notice "node_ids $node_ids"
		foreach node_id $node_ids {
		    set ls_coimtabs [db_list q "select nome_colonna
                                              from coimtabs 
                                             where nome_tabella = :csv_name
                                          order by ordinamento"]
		    
		    foreach nome_colonna_nodo $ls_coimtabs {
			
			set $nome_colonna_nodo [ah_xml_get_text_value -optional $node_id $nome_colonna_nodo] 
			
			set nodo_i [ah_xml_get_node_id $root_id $nome_colonna_nodo]
			ns_log notice "nodo_i $nodo_i"
			if {$nodo_i eq ""} {
			    append err_log "L'intero lotto &egrave; stato scartato in quanto il tracciato non è nel formato/ordine corretto."
			    break
			}		    
			
			if {$nome_colonna_nodo eq "cod_manutentore"} {
			    if {[string equal $cod_manutentore_chk ""]} {
				set cod_manutentore_chk [set $nome_colonna_nodo]
			    }
			    if {([set $nome_colonna_nodo] != $cod_manutentore_chk || [string equal [set $nome_colonna_nodo] ""]) } {
		    ...
		    if {[string equal $err_log ""]} {
			set nome_tabella "XRCEE1CARI_"
			#set cod_manu_tab [ah_xml_get_text_value -optional $node_id "cod_manutentore"]
			...
			while {$i < $count_fields} {
			    ...
			    set nome_variabile [lindex $file_fields($i) 0]
			    
			    set colonna [set $nome_variabile]
			    ns_log notice "cari-rce-xml-tipo-1  $colonna nome_variabile [set $nome_variabile] $nome_variabile"
		...
		};#fine foreach su node_id
	    }
	    
	    if {[string equal $err_log ""]} {
		if {[db_0or1row sel_cari_manu "select '1' from coimcari_manu where cod_manutentore = :cod_manutentore limit 1"]} {
		...
		while {$i < $count_fields} {
		    ...
		    lassign $file_fields($i) nome_colonna denominazione type dimensione obbligatorio default_value range_value
	    ...
	    ns_log notice "test luca OK 1 file_table_id $file_table_id file_inp_id $file_inp_id"
	}
	...
	if {[string equal $err_log ""]} {
		...
		while {$i < $count_fields} {
		    ...
		    lassign $file_fields($i) col_name denominazione type dimension obbligatorio default_value range_value
		    incr i
			...
			if {(![string equal $data_rottamaz_gen ""]
				...
				date {
				    ...
				    ns_log notice "LUCAR test2.1 col_name $col_name colonna $colonna"
					...
					incr errori
					ns_log notice "LUCAR test2.2. nome_tabella_anom $nome_tabella_anom desc_errore $desc_errore col_name $col_name"
				...
				numeric {
				    ...
				    lassign $int_dec intero decimale
				    if {[iter_edit_num $colonna $decimale] != "Error"} {
					set element_int_dec [split $colonna \.]
					lassign $element_int_dec parte_intera parte_decimale
				...
				varchar {
					...
					set desc_errore "Il campo $col_name deve essere al massimo di $dimension caratteri"
					incr errori
					ns_log notice "LUCAR test2.3 nome_tabella_anom $nome_tabella_anom  desc_errore $desc_errore"
		...
		ns_log notice "LUCAR test1.1"

		if {$errori > 0} {
		    # $num_anom_max
		    if {$errori > 100} {
			ns_log notice "LUCAR test1.1.1"
			...
			if {![string equal $toponimo ""] && ![string equal $indirizzo ""] && ![string equal $cod_comune_chk ""]} {
				...
				if {[db_0or1row sel_viae_check "select cod_via
				    ...
				    if {![db_0or1row q "select 1 
                                                          from $nome_tabella_anom 
                                                         where cod_manutentore = :cod_manutentore 
                                                           and id_riga         = :id_riga 
                                                           and nome_colonna    = 'toponimo'
                                                          limit 1"]} { 
			...
			ns_log notice "LUCAR test1.1.2 nome_tabella $nome_tabella numero_anomalie $errori"
			db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'P' where id_riga = :id_riga"
		    }
		    ns_log notice "LUCAR test1.1.2.1"
		} else {
			...
			lassign $dati_analizzatore marca_analizzatore modello_analizzatore matricola_analizzatore;#rom03
			...
			set cod_strumento_02_dimp "";#rom03
			if {![string is space $cod_strumento_02]} {#rom03 Aggiunta if e suo contenuto
			    
			    # Il deprimometro e' obbligatorio solo in un caso quindi devo gestire
			    # il fatto che possa essere lasciato vuoto.
			    
			    set dati_deprimometro [split $cod_strumento_02 "-"]
			    
			    lassign $dati_deprimometro marca_deprimometro modello_deprimometro matricola_deprimometro
			    
			    set marca_deprimometro     [string trim $marca_deprimometro]
			    set modello_deprimometro   [string trim $modello_deprimometro]
			    set matricola_deprimometro [string trim $matricola_deprimometro]
			...
			set marca_analizzatore     ""
			set modello_analizzatore   ""
			set matricola_analizzatore ""
			set cod_strumento_01_dimp  ""
			set cod_strumento_01       ""
			
			set marca_deprimometro     ""
			set modello_deprimometro   ""
			set matricola_deprimometro ""
			set cod_strumento_02_dimp  ""
			set cod_strumento_02       ""
		    }
			...
			set cod_citt [db_string sel_citt_check "select cod_cittadino as cod_citt
                                                             ...
                                                             --rom03 and upper(indirizzo) = upper(:indirizzo_resp)
	 ...
	 ns_log notice "luca OK 2"   
	    ...
	    if {[string equal $nome_tabella ""] && ![string equal $root_id ""]} {
		ns_log notice "luca OK 3 file_table_id $file_table_id file_inp_id $file_inp_id"
		close $file_table_id
		#close $file_inp_id
	    }
	    ns_log notice "luca OK 4"
	}
	...
	if {[string equal $err_log ""]} {
		...
		file delete $file_esi
		file delete $file_err
		file delete $file_tot
		...
		db_foreach sel_righe_buone "select * from $nome_tabella where flag_stato is null" {
		    ...
		    if {[string equal $data_rottamaz_gen ""]} {
			if {!($coimtgen(regione) in [list "MARCHE" "CAMPANIA"])} {
			...
			ns_log notice "iter-cari-rce-xml-tipo-1-procs Gacalin - cod_impianto:$cod_impianto - cod_impianto_catasto:$cod_impianto_catasto - cod_responsabile:$cod_responsabile
potenza_impianto $potenza_impianto - data_scadenza_autocert $data_scadenza_autocert - sw_data_controllo_ok $sw_data_controllo_ok combustibile $combustibile"
			...
			#rom inizio
			ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 0"
			if {$coimtgen(ente) eq "CRIMINI"}  {
			...
			} elseif {$coimtgen(ente) eq "PRI"} {#ric03 elseif e suo contenuto
				...
				set data_scadenza_autocert [db_string query "select :data_controllo::date + interval '$year_scadenza year'" -default ""]
				    ...
				    ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 0.1"
			    if {$sw_data_controllo_ok == "t"} {
			    ...
			    ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 0.2"
			...
			if {$flag_portafoglio == "T"
				...
				if {(![string equal $cod_impianto_catasto ""]) && ([db_0or1row sel_dimp_check_data_controllo ""] == 1)} {
				    ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 1"
				    set tariffa_reg "7"
				    set importo_contr "0.00"
				    db_1row sel_dual_cod_dimp ""
				} else {
				    set tariffa_reg "7"
				    ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 2"
					...
					ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 3"
					...
					ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 5"
				...
				ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 6"
				db_1row sel_dual_cod_dimp ""
				set tariffa_reg "8"	
				set importo_contr "0.00"
			    }
			} else {
			    ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 7"
			    db_1row sel_dual_cod_dimp ""
			    set importo_contr ""
			    set tariffa_reg ""
			}
			...
			if {$err_count == 0} {
				...
				#rom04 Il campo idoneita_locale se inserito da iter ha il valore E se e' N.C.
				#      Dal tracciato del caricamento invece ha il valore C se e' N.C.
				#      Per non dover comunicare niente ai manutentori e non dover modificare
				#      altri programmi forzo il valore da C a E nel caso il locale sia N.C.
				
				if {[string equal $idoneita_locale "C"]} {#rom04 Aggiunta if e suo contenuto
				    set idoneita_locale "E"
				}
				ns_log notice "LUCAR test3"

				db_dml upd_aimp_date "";#rom04 Vado ad aggiornare le date di scadenza e ultimo RCEE sulla coimaimp

				ns_log notice "iter-cari-rce-xml-tipo-1-procs LUCAR 8"
				db_1row q "select pot_utile_nom as potenza
                                             from coimgend
                                            where cod_impianto = :cod_impianto
                                              and gen_prog     = :gen_prog";#rom05
				ns_log notice "LUCAR test4"
	...
	ns_log Error "iter_cari_rce_xml_tipo_1: $error_msg"
    }

    ns_log Notice "Fine della proc iter_cari_rce_xml_tipo_1"
    return
}
