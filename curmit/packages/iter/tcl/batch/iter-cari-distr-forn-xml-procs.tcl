ad_proc iter_cari_distr_forn_xml {
    {
	-cod_batc      ""
        -id_utente     ""
	-file_name     ""
    ...
    @author          
    @creation-date   
    @cvs-id          iter_cari_distr_forn_xml
    ...
    ns_log notice "start iter-cari-distr-forn-xml-procs"
    ns_log notice "iter-cari-distr-forn-xml-procs cod_batc=$cod_batc id_utente=$id_utente file_name =$file_name "
	...
	set file_inp_name  "Caricamento-distr-forn-xml-input"
	set file_inp_name  [iter_temp_file_name -permanenti $file_inp_name]
	set file_esi_name  "Caricamento-distr-forn-xml-esito"
	set file_esi_name  [iter_temp_file_name -permanenti $file_esi_name]
	set file_out_name  "Caricamento-distr-forn-xml-caricati"
	set file_out_name  [iter_temp_file_name -permanenti $file_out_name]
	set file_err_name  "Caricamento-distr-forn-xml-scartati"
...
#	}
	...
	ns_log notice "iter-cari-distr-forn-xml: test 1 file_inp $file_inp file_name $file_name file_inp_name $file_inp_name"
	...
	if {[catch {set file_esi_id [open $file_esi w]}]} {
	    ns_log notice "File di esito caricamento non aperto: $file_esi"
	    break
	}
	...
	if {[catch {set file_out_id [open $file_out w]}]} {
	    ns_log notice "File csv dei record caricati non aperto: $file_out"
	    break
	}
	# dichiaro di scrivere in formato iso West European
	...
	if {[catch {set file_err_id [open $file_err w]}]} {
	    ns_log notice "File csv dei record scartati non aperto: $file_err"
	    break
	}
	# dichiaro di scrivere in formato iso West European
	...
	set elenco_campi [list "name_distributor" \
			      "natura_giurid" \
			      "utente_cogn_rag_soc" \
			      "utente_nome" \
			      "utente_cf" \
	...
	with_catch msg_err_curl {
	    ...
	    set root_id  [ah_xml_get_root_id $response];#equivale a caricamento_forniture
	} {
	    set motivo_scarto "Il file &egrave; stato scartato in quanto il tracciato non è nel formato/ordine corretto. 1"
	    set root_id ""
	    set carica "N"
	}
	...
	if {$carica eq "S"} {
		...
		set motivo_scarto "Il file &egrave; stato scartato in quanto il tracciato non è nel formato/ordine corretto. 2"
		set node_ids ""
		set carica "N"
	    }
	}
	if {$carica eq "S"} {
		...
		foreach child [$node_id childNodes] {
			...
			set motivo_scarto "Il file &egrave; stato scartato in quanto il tracciato non è nel formato/ordine corretto. Fornitura N. $num_fornitura: non è previsto il tag $nome_tag"
			set carica "N"
		    }
		}
	    }
		...
		foreach nome_colonna_nodo $elenco_campi {
		    ns_log notice "iter-cari-distr-forn-xml: node_id $node_id nome_colonna_nodo $nome_colonna_nodo"
		...
		set error_descr ""
		set toponimo_tipo [string toupper [string trimright $toponimo_tipo]]
		set user_fuel [string toupper [string trimright $combustibile_tipo]]
		set contract  [string trimright $contratto_tipo]
		
		set len_pdr [string length $pdr];#but05
		set segno "";#but07
		set len [string length $toponimo_civico];#but07
		set max_decimali 2
		set ctr_virgole [regsub -all "," $combustibile_consumo "," campo_consum];#but06
		set pos_virg [string first "," $combustibile_consumo];#but06
		set decimali [string range $combustibile_consumo [expr $pos_virg + 1] end];#but06
		set len_decimali [string length $decimali];#but06

		if {$carica == "S"} {
		    if {[string equal $utente_cogn_rag_soc ""]} {
			set carica        "N"
		...
		if {$carica == "S"} {
		    if {[string equal $toponimo_nome ""]} {
			set carica        "N"
			set motivo_scarto "Nome toponimo non valorizzato"
		    }
		}

		# toponimo_cap non obbligatorio

		# toponimo_cap non obbligatorio
		if {$carica == "S"} {
		    if {![string equal $toponimo_cap ""]
			&&  ![string is integer $toponimo_cap]
		    } {
			set carica        "N"
		...
		if {$carica == "S"} {
		    if {[string equal $comune_nome ""]} {
			set carica        "N"
			set motivo_scarto "Comune non valorizzato"
		    } else {
			set denominazione $comune_nome
		...
		if {$coimtgen(flag_viario) == "T"
		    ...
		    set descr_topo  $toponimo_tipo
		    set descrizione $toponimo_nome
		...
		# ROM 13/10/2023 Controllo se esiste gia' un impianto con utente_cf e indirizzo toponimo_nome.
		# ROM            Al momento Sandro ha detto di non controllare anche il civico toponimo_cap.
		if {[db_0or1row q "select 1
                                    ...
                                    where upper(v.descrizione) = upper(:toponimo_nome)
                                      and upper(v.descr_topo)  = upper(:toponimo_tipo)
                                      and v.cod_comune       = :cod_comune
                              --      and a.numero           = :toponimo_cap
                                      and c.cod_fiscale      = :utente_cf
                                    limit 1"]} {
		    set carica "N"
		    set motivo_scarto "Esiste già un impianto con Cod.Fiscale $utente_cf all'indirizzo $toponimo_nome"
		}

		#Se ho stesso pod,pdr e stessa via non carico l'impianto
		if {[db_0or1row q "select 1
                                     from coimaimp a
                                     left join coimviae v on v.cod_via       = a.cod_via
                                    where a.pod = :pod
                                      and a.pdr = :pdr
                                      and upper(v.descrizione) = upper(:toponimo_nome)
                                      and upper(v.descr_topo)  = upper(:toponimo_tipo)"]} {
		    set carica "N"
		    set motivo_scarto "Esiste già un impianto con POD $pod, PDR $pdr all'inidizzo $toponimo_nome"
		}

		# Se c'è già la persona con stesso pod e pdr non carico
		if {[db_0or1row q "select 1
                                     from coimaimp a
                                     left join coimcitt c on c.cod_cittadino = a.cod_responsabile
                                    where a.pod = :pod
                                      ...
                                      and c.cod_fiscale      = :utente_cf"]} {
		    set carica "N"
		    set motivo_scarto "Esiste già un impianto con POD $pod, PDR $pdr e Cod.Fiscale $utente_cf"
		}

		
		# ws_prefisso_telefonico non obbligatorio
		# ws_numero_telefonico non obbligatorio
		set ws_numero_telefonico ""
		set nome      $utente_nome
		set cognome   $utente_cogn_rag_soc
		...
		if {$carica == "S"} {
		    ...
		    if {[string length $utente_cogn_rag_soc] > 100} {
			set cognome  [string range $utente_cogn_rag_soc 0 99]
		    }
		    if {[string length $utente_nome] > 100} {
			set nome     [string range $utente_nome    0 99]
		    }
		    ...
		    set comune        $comune_nome

		    set indirizzo    "$toponimo_tipo $toponimo_nome"
		    ...
		    set numciv_espciv_list [iter_separa_numciv_espciv $toponimo_cap]
			...
			set cod_fiscale      $utente_cf
			set cod_piva         $utente_piva
			set telefono         ""
			set ws_prefisso_telefonico ""
			...
			ns_log notice "iter-cari-distr-forn-xml swc_solo_aimp $swc_solo_aimp"
			if {$swc_solo_aimp != "S"} {
			    set cod_inco         [db_string sel_dual_cod_inco ""]
			    set cod_cinc         [db_string sel_cinc ""]
			    ns_log notice "iter-cari-distr-forn-xml cod_inco $cod_inco cod_cinc $cod_cinc"
			    ...
			    ns_log notice "iter-cari-distr-forn-xml ins_inco: cod_inco $cod_inco"
		...
		set ind 0
		foreach column_name $elenco_campi {
	...
	ns_log notice "iter-cari-distr-forn-xml-procs.tcl close0"
	
	#close $file_inp_id
	
	ns_log notice "iter-cari-distr-forn-xml-procs.tcl close1"
	
	close $file_esi_id
	
	ns_log notice "iter-cari-distr-forn-xml-procs.tcl close2"
	
	close $file_out_id
	
	ns_log notice "iter-cari-distr-forn-xml-procs.tcl close3"
	
	close $file_err_id
	
	ns_log notice "iter-cari-distr-forn-xml-procs.tcl close4"
	...
	ns_log Error "iter_cari_distr_forn_xml: $error_msg"
    }



    ns_log notice "end iter-cari-distr-forn-xml-procs"
