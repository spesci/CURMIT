ad_proc iter_dist_cari {
	...
	db_transaction {
	    ...
	    lappend head_cols "Tipo combustibile"
	    ...
	    lappend file_cols "inp_tipo_combustibile"
		...
		set ws_tipo_combustibile   [string trim    $inp_tipo_combustibile]
		...
		if {$carica == "S"} {
		    if {![string equal $ws_tipo_combustibile ""]} {
			set descr_comb $ws_tipo_combustibile
			db_1row sel_comb_check ""
			# con la query precedente ho valorizzato ctr_comb
			# e cod_combustibile con il max(cod_combustibile)
			if {$ctr_comb == 0} {
			    set carica        "N"
			    set motivo_scarto "Combustibile non esistente in tabella Combustibili"
			}
			if {$ctr_comb > 1} {
			    set carica        "N"
			    set motivo_scarto "Trovati più combustibili con questa denominazione"
			}
		    }
		}
		...
		if {$carica == "S"} {
		    ...
		    if {$coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO"} {
			# Per Udine e Gorizia non va mai creato
			set swc_solo_aimp "S"
		    }
		    if {$swc_solo_aimp != "S"} {
	...
	ns_log Error "iter_dist_cari: $error_msg"
    }
    return
}
