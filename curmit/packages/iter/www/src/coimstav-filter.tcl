...
#rom01set page_title   "Selezione appuntamenti assegnati per stampa avvisi"
set page_title   "Stampa multipla avvisi di ispezione";#rom01
...
if {[form is_valid $form_name]} {
    ...
    set f_da_data_verifica [element::get_value $form_name f_da_data_verifica]
    set f_a_data_verifica [element::get_value $form_name f_a_data_verifica]
   ...
   if {$coimtgen(ente) ne "PUD" && $coimtgen(ente) ne "PGO"} { 
        if {[string is space $f_cog_tecn]} {
	    element::set_error $form_name f_cog_tecn "Dato Obbligatorio"
	    incr error_num
	}
   }
	...
	if {$f_da_data_verifica == 0} {
	    element::set_error $form_name f_da_data_verifica "deve essere una data valida"
	    incr error_num
	}
    }
	...
	if {$f_a_data_verifica == 0} {
	    element::set_error $form_name f_a_data_verifica "deve essere una data valida"
	    incr error_num
	}
    }
...
#dpr74
    set link_list [export_url_vars caller funzione nome_funz nome_funz_caller cod_cinc f_tipo_data f_data f_cod_impianto f_tipo_estrazione f_anno_inst_da f_anno_inst_a f_cod_comb f_cod_enve f_cod_tecn f_cod_comune f_cod_via f_descr_topo f_descr_via f_num_max f_cod_area flag_scaduto id_stampa f_da_data_verifica f_a_data_verifica flag_tipo_impianto]

    set return_url "coimstav-list?$link_list&flag_avviso=S"
