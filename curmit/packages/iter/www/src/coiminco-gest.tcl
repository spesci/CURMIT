ad_page_contract {
    ...
    @param funzione D=Delete V=View A=Assegna C=Conferma E=Effettua N=Annulla
...
set mail_ind_mittente     [parameter::get_from_package_key -package_key iter -parameter mail_ind_mittente -default ""]
set mail_ind_dest_forzato [parameter::get_from_package_key -package_key iter -parameter mail_ind_dest_forzato -default ""]
...
if {$flag_cod_tecn == "t" || $flag_cod_enve == "t"} {
...
set link_list_script {[export_url_vars last_cod_inco caller nome_funz_caller nome_funz flag_aimp cod_impianto url_aimp url_list_aimp]&[iter_set_url_vars $extra_par]}
...
if {$caller == "index"} {
    ...
    set context_bar  [iter_context_bar \
			  ...
			  [list coiminco-list?$link_list "Lista Incontri"] \
			  "$page_title"]
}
...
if {$flag_cod_enve == "t" || $flag_cod_tecn == "t"} {
...
element create $form_name stato_imp \
    -label   "Stato Impianto" \
    -widget   select \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
    -optional \
    -options {{{} {}} {Annullato L} {Nonattivo N} {Rottamato R}}

#sim01
element create $form_name flag_blocca_rcee \
    -label   "Blocca inserimento RCEE" \
    -widget   select \
    -datatype text \
...
set prep_link_alle {
    ...
    if {![string is space $cod_documento_01] && [db_0or1row sel_docu ""] == 1} {
    ...
    if {![string is space $cod_documento_02] && [db_0or1row sel_docu ""] == 1} {
	set link_docu [export_url_vars cod_documento cod_impianto url_aimp url_list_aimp nome_funz_caller]&nome_funz=docu
...
if {[form is_request $form_name]} {
    ...
    element set_properties $form_name stato_imp        -value $stato_imp
    element set_properties $form_name flag_blocca_rcee -value $flag_blocca_rcee;#sim01
...
if {[form is_valid $form_name]} {
    ...
    set stato_imp         [element::get_value $form_name stato_imp]
    set flag_blocca_rcee  [element::get_value $form_name flag_blocca_rcee];#sim01

    if {$flag_cod_enve == "f" && $flag_cod_tecn == "f"} {
	...
	if {$cod_noin eq "2" || $cod_noin eq "9" || $cod_noin eq "14" || $cod_noin eq "16" || $cod_noin eq "17" || $cod_noin eq "11" || $cod_noin eq "10" || $cod_noin eq "18"} {
	    ...
	    if {$stato_imp ne ""} {
		element::set_error $form_name stato_imp "Stato Impianto non va specificato per motivi diversi da Imp.Ines., Imp.Disat., Imp.Non Sog."
		incr error_num
	    }
	}
    }
    if {$funzione != "D" && $funzione != "V"} {
	...
	if {$funzione == "A" || $funzione == "C" || $funzione == "E"} {
		...
		if {[string equal $cog_tecn ""] && [string equal $nom_tecn ""]} {
		...
		if {$data_verifica == 0} {
		    ...
		    if {$data_verifica < $dt_inizio_cinc || $data_verifica > $dt_fine_cinc} {
    ...
    if {$funzione == "N"} {
	if {[string equal $note ""] && [string equal $cod_noin ""]} {
    ...
    switch $funzione {
	    ...
	    if {$stato_old != "5"} {
		if {$cod_opve_old != $cod_opve && $data_verifica_old != $data_verifica && $ora_verifica_old != $ora_verifica} {
	    ...
	    # se c'è il codice ispettore e la nota o la motivazione dell'annullamento, invio mail all'ispettore
	    if {$cod_opve ne "" && ($note ne "" || $cod_noin ne "")} {
		db_1row query "select to_char(current_date, 'dd/mm/yyyy') as oggi"
		db_1row query "select o.cognome||' '||coalesce(o.nome, '') as nome_ispettore
                                    , u.e_mail as email
                                 from coimopve o left outer join coimuten u on u.id_utente = o.cod_opve
                                where o.cod_opve = :cod_opve"
		if {![string is space $mail_ind_dest_forzato]} {
		    set email $mail_ind_dest_forzato
		}
		if {$email eq "" || $email eq "." || $mail_ind_mittente eq ""} {
		    ...
		    db_1row query "select cod_impianto, cod_inco as cod_appuntamento
                                        , to_char(data_verifica, 'dd/mm/yyyy') as data_appuntamento
                                     from coiminco
                                    where cod_inco = :cod_inco"
		    db_1row query "select cod_impianto_est as codice_impianto
		                 ...
		                 from coimaimp a left outer join coimcitt d on d.cod_cittadino = a.cod_responsabile
		    ...
		    if {$note ne "" && $cod_noin ne ""} {
			set motivazione_annullamento "$descr_noin - $note"
		    } else {
			set motivazione_annullamento "$descr_noin $note"
		    }
		    ...
		    set indirizzo_impianto "$topo $via$virgola $numero$barra$esponente$sc$scala$pi$piano$int$interno$loc$localita $cap $comune"
		    
		    # Uso coimdesc(nome_ente) perche' Ucit usa UCIT S.R.L.
		    set mail_text "Data: $oggi.\n\nE' stato annullata da $coimdesc(nome_ente) la seguente ispezione:\n\nID: $cod_appuntamento.\nCodice: $codice_impianto. Indirizzo: $indirizzo_impianto. Responsabile: $resp.\nData pianificata: $data_appuntamento.\nCon la seguente motivazione: $motivazione_annullamento.\n\nDistinti Saluti.\n$coimdesc(nome_ente) \n"

		    # Uso ns_sendmail e non acs_mail_lite perchè per Udine e Gorizia non funziona
		    ns_sendmail $email $mail_ind_mittente "Appuntamento annullato" $mail_text
		    #acs_mail_lite::send -from_addr $mail_ind_mittente -to_addr $email -subject "Appuntamento annullato" -body $mail_text
		}
	    }
	}
    }
    ...
    switch $funzione {
        D {set return_url   "coiminco-list?$link_list"}
        V {set return_url   "coiminco-list?$link_list"}
        A {set return_url   "coiminco-gest?funzione=V&$link_gest"}
        C {set return_url   "coiminco-gest?funzione=V&$link_gest"}
        E {set return_url   "coiminco-gest?funzione=V&$link_gest"}
        N {set return_url   "coiminco-gest?funzione=V&$link_gest"}
    }
