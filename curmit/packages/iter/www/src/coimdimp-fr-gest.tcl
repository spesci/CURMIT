ad_page_contract {
    ...
    rom13            inserire RCEE se con la stessa targa e' presente un impianto del caldo ibrido.

    rom12 09/07/2021 Per Regione Marche e Campania gli impianti con potenza di 100 Kw hanno giustamente
    rom12            la tariffa di 14 Euro ma la scadenza era erronemanete di 2 anni, ora e' stata 
    rom12            messa a 4 anni. La segnalazione e' stata fatta solo da Regione Marche ma Sandro
    rom12            ha detto che la modifica va bene anche per la Campania in quanto e' un errore
    rom12            a livello nazionale.

    sim38 13/04/2021 Corretto un errore nel caso di potenza nulla sulla Regione Marche.
    sim38            Prima non capitava perchè il campo era obbligatorio e sempre valorizzato. 
    ...
    rom10 23/12/2002 Su richiesta di Regione Marche e' stata data la possibilita' agli Installatori di poter inserire
    rom10            gli RCEE se viene scelto come motivo di compilazione: Prima messa in servizio, Sostituzione del
    rom10            generatore e Ristrutturazione dell'impianto.
    ...
    sim37 30/09/2020 Su richiesta della Regione Marche è stato cambiato il messaggio di errore
    sim37            in caso di impianto non attivo. Va bene per tutti gli enti.
    
    rom08 03/08/2020 La data di scadenza deve essere calcolata in base alla maggiore tra la potenza
    rom08            frigorifera e la termica, come da telefonata di Sandro con Regione Marche.

    rom07 28/07/2020 Sandro ha detto di mettere i giorni limite per inserire l'RCEE come per il caldo su Salerno.

    rom06 26/06/2020 Cambiata frase per gli errori non bloccanti del campo err_rcee su richiesta di Sandro.

    rom05 06/05/2020 Corretta dicitura sul messaggio di errore sul saldo del portafoglio:
    rom05            tolta didicura areaoperativa.curit.it

    rom04 04/05/2020 Sandro ha detto che per il freddo non deve esserci il controllo sul patentino su
    rom04            tutti gli enti.

    sim36 07/02/2020 Tutti gli enti devono avere la data prossima manutenzione obbligatoria

    sim35 14/01/2020 Sandro per il momento dice di tenere le scadenze di Salerno uguali alla Regione Marche

    rom03 09/07/2019 La Provincia di Salerno non vuole obbligatorio il tecnico

    sim34 09/12/2019 Corretto search sugli operatori per escludere i non attivi

    sim33 02/12/2019 Gestito i casi in cui data_costruz_gen e data_insta sono null e quindi non vanno editate

    sim32 27/11/2019 Corretto errore su editing di data_costruz_gen.

    gac10 22/11/2019 Se non ho inserito tutti  i campi obbligatori posso entrare in sola visualizzazione
    
    sim31 11/10/2019 Gestito sulla regione marche l'esenzione per il secondo generatore
    ...
    gac09 13/02/2019 Su richiesta della regione marche ho tolto l'opzione NC per i campi della sezione B.

    gac08 22/01/2019 Sandro mi ha detto che il campo "Potenza frigorifera nominale in raffrescamento"
    gac08            deve esporre la colonna pot_focolare_nom della tabella coimgend
    
    gac07 21/01/2019 Aggiunti campi acquisti, acquisti2, scorta_o_lett_iniz, scorta_o_lett_iniz2, scorta_o_lett_fin 
    gac07            e scorta_o_lett_fin2, aggiunto anche campi elettricità.

    gac06 14/01/2019 Apportato alcune modifiche per regione marche, impostato alcuni campi non modificabili.
    
    gac05 16/11/2018 Apportato modifiche alla parte finale dell'rcee di tipo 2 solo per regione marche 
   ...
   Mrom02 04/10/2018 Non è possibile inserire rcee tipo 2 su impianti del freddo che sono associati 
   Mrom02            a impianti del caldo che hanno il campo flag_ibrido valorizzato con Sì.
   Mrom02            Per il momento blocco solo le Marche.

    sim28 03/08/2018 Il programma deve proporre la Potenza utile frigorifera e non la potenza assorbita

    gac04 11/07/2018 Sandro segnalava che il cod_tprc (tipo_controllo) non veniva salvato correttamente, il
    gac04            problema era che veniva settato di default 'CADALLE' quindi anche se a db il tipo controllo
    gac04            veniva salvato correttamente quando si entrava in V si vedeva sempre 'CADALLE'

   Msim28 19/04/2018 Gestito per la Regione Marche l'inserimento della tipologia controllo e la gestione della
   Msim28            eventuale esenzione

    gab03 12/04/2018 Gestione del Multiportafoglio: in tutte le url di chiamata ai web service del portale 
    gab03            passo il nome dell'ente portafoglio.

    sim27 01/06/2018 Gli enti della Regione Calabria non vogliono più il controllo sul patentino.

    sim26 21/05/2018 Il riferimento pagamento deve essere in readonly se è attivo il portafoglio

    sim25 21/05/2018 Ucit fa pagare il contributo regionale anche sulle prime installazioni

    sim24 08/05/2018 Ucit può usare il portafoglio solo per controlli dal 1 Maggio 2018 in poi
    ...
    gac02            carattere perchè si riferisce ad una lettera estranea alla matricola

    rom01 26/01/2018 Gestita la possibilità di inserire più prove fumi per dichiarazione del freddo.

    sim22 18/01/2017 Udine, Gorizia e Pordenone non vogliono più il controllo sul patentino
    ...
    gac01            Provincia e Comune di Ancona. Ora non inserisce più una attività in sospeso
    ...
    sim15            di un rcee su un impianto che ha già un rcee nello stesso giorno su un 
    sim15            altro generatore del medesimo impianto, la tariffa deve essere 0.
    sim15            Inoltre se l'impianto non e' funzionante la tariffa è 0. 
    ...
    sim06 11/03/2016 Se è attivo il portafoglio va abilitato solo il bollettino virtuale  e il
...
set ente_portafoglio [db_get_database];#gab03
...
if {$funzione eq "I"} {
    ...
    if {$stato_imp ne "A" } {
#sim37        iter_return_complaint "
#sim37        Funzione impossibile per impianti annullati/Non attivi/Rottamati"  
	iter_return_complaint "Sarà possibile trasmettere RCEE e altri moduli solo dopo la riattivazione/validazione dell'impianto da parte dell'ente";#sim37
	ad_script_abort
    }
}
#fine modifica
...
set err_rcee "";#gac05
set msg_rcee "";#gac05
set warning_cont_rend "";#gac06

#Mrom02 Non è possibile inserire rcee tipo 2 su impianti del freddo che sono associati a impianti del caldo
#Mrom02 che hanno il campo flag_ibrido valorizzato con Sì. Per il momento blocco solo le Marche.
#rom13  Aggiunta condizione 1==0 per non entrare piu' nel blocco al momento.
if {$coimtgen(regione) eq "MARCHE" && $funzione eq "I" && 1==0} {#Mrom02 if e contenuto
    if {[db_0or1row q "select b.cod_impianto_est as cod_impianto_est_ibrido 
                         from coimaimp a
                            , coimaimp b 
                        where a.cod_impianto = :cod_impianto 
                          and b.flag_tipo_impianto = 'R' 
                          and a.targa = b.targa 
                          and b.flag_ibrido = 'S'"] == 1} {
	iter_return_complaint "Impossibile inserire RCEE di Tipo 2 con generatori ibridi. Inserire RCEE relativo al solo gruppo termico (tipo 1) con codice impianto $cod_impianto_est_ibrido"
    }
}
...
set controllo_tariffa {#sim07
    ...
    if {[string equal $data_controllo ""]} {#Msim28 in caso di refresh del tipo controllo con la data controllo non inserita, il programma andava in errore
    ...
    #sim15 verifico se già esiste un rcee sull'impianto con la stessa data controllo ma su un generatore diverso.
	...
	set tariffa 0
	set esente "t"
    } elseif {$coimtgen(regione) eq "MARCHE" && $rcee_su_secondo_gen eq "t"} {#sim31
	set tariffa 0
	set esente "t"
    } elseif {$coimtgen(regione) eq "MARCHE" && $cod_tprc ne ""} {#Msim28
...
if {$funzione == "I" || $funzione == "M"} {
    set fstato 0;#sim34
    #sim34 set cerca_opma [iter_search $form_name [ad_conn package_url]/src/coimopma-list [list cod_manutentore cod_manutentore dummy cod_opmanu_new f_cognome nome_opma f_nome cognome_opma]]
    set cerca_opma [iter_search $form_name [ad_conn package_url]/src/coimopma-list [list cod_manutentore cod_manutentore dummy cod_opmanu_new f_cognome nome_opma f_nome cognome_opma] &fstato=$fstato];#sim34
} else {
    set cerca_opma ""
}
...
element create $form_name stagione_risc2 \
    ...
    -html    "size 20 maxlength 40 $readonly_fld {} class form_element" \
    -optional
#gac07 aggiunto campi acquisti, acquisti2, scorta_o_lett_iniz, scorta_o_lett_iniz2, scorta_o_lett_fin e scorta_o_lett_fin2 e campi elettricita
...
if {$flag_mod_gend == "S"} {
    if {$coimtgen(regione) eq "MARCHE"} {#gac06 parte per regione marche
...
#gac06    element create $form_name costruttore \
#gac06	-label   "costruttore" \
#gac06	-widget   select \
#gac06	-datatype text \
#gac06	-html    "$disabled_fld {} class form_element $html_per_costruttore" \
#gac06	-optional \
#gac06	-options [iter_selbox_from_table coimcost cod_cost descr_cost]
	...
	element create $form_name costruttore -widget hidden -datatype text -optional;#gac0
	...
	element create $form_name combustibile -widget hidden -datatype text -optional;#gac06
...
#gac06    element create $form_name data_insta \
#gac06	-label   "Data installazione" \
#gac06	-widget   text \
#gac06	-datatype text \
#gac06	-html    "size 15 maxlength 100 readonly {} class form_element" \
#gac06	-optional
        element create $form_name data_insta -widget hidden -datatype text -optional;#gac06
...
#gac06    element create $form_name data_costruz_gen \
#gac06	-label   "Data costruzione generatore" \
#gac06	-widget   text \
#gac06	-datatype text \
#gac06	-html    "size 10 readonly {} class form_element" \
#gac06  -optional

	element create $form_name data_costruz_gen -widget hidden -datatype text -optional;#gac06
	
    
#gac06    element create $form_name marc_effic_energ \
#gac06	-label   "Marcatura efficienza energetica" \
#gac06	-widget   text \
#gac06	-datatype text \
#gac06	-html    "size 4 maxlength 10 $readonly_fld {} class form_element" \
#gac06	-optional
	element create $form_name marc_effic_energ -widget hidden -datatype text -optional;#gac06
	...
	#gac08 scommentato nic03: Sandro mi ha detto che potenze frigorifera nominale in riscaldamento deve corrispondere
	#gac08 a questo campo della tabella
...
#gac06	element create $form_name potenza \
#gac06	    -label   "Potenza" \
#gac06	    -widget   text \
#gac06	    -datatype text \
#gac06	    -html    "size 10 maxlength 10 readonly {} class form_element" \
#gac06	    -optional
	element create $form_name potenza -widget hidden -datatype text -optional;#gac06
...
#gac06	-optional
	...
	element create $form_name costruttore -widget hidden -datatype text -optional;#gac06
...
#gac06    element create $form_name combustibile \
#gac06	-label   "Combustibile" \
#gac06	-widget   text \
#gac06	-datatype text \
#gac06	-html    "size 15 maxlength 100 readonly {} class form_element" \
#gac06	-optional
...
#gac06	-widget   text \
#gac06	-datatype text \
#gac06	-html    "size 4 readonly {} class form_element" \
#gac06	-optional
...
if {$coimtgen(regione) eq "MARCHE"} {#gac00 aggiunta if else e contenuto di if per togliere la option NC per regione marche
...
element create $form_name fr_assorb_recupero \
    ...
    -widget   text \
    -datatype text \
    -html    "readonly {} size 2 class form_element" \
    -optional
...
element create $form_name fr_prova_modalita \
    ...
    -widget   select \
    -options  {{Raffrescamento F} {Riscaldamento C}  {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
    -optional
...
if {$coimtgen(regione) ne "MARCHE"} {#gac05
...
element create $form_name is_warning_p     -widget hidden -datatype text -optional;#gac05
element set_properties $form_name is_warning_p      -value $is_warning_p;#gac05

if {$coimtgen(regione) eq "MARCHE"} {#gac05 aggiunta if else e loro suo contenuto
    ...
    element create $form_name err_rcee -widget hidden -datatype text -optional;#gac05
    element create $form_name msg_rcee -widget hidden -datatype text -optional;#gac05
    element create $form_name warning_cont_rend -widget hidden -datatype text -optional;#gac06
}
...
element set_properties $form_name pot_ter_nom_tot_max -value $pot_ter_nom_tot_max;#gac02
...
if {[form is_request $form_name]} {
    ...
    element set_properties $form_name cod_tprc         -value $cod_tprc;#gac04"CADALLE";#sim43

    element set_properties $form_name err_rcee          -value "";#gac05
    element set_properties $form_name msg_rcee          -value "";#gac05
    element set_properties $form_name warning_cont_rend -value "";#gac06
	...
	set pot_focolare_nom ""
        set pot_utile_lib    ""
	#sim29        set pot_utile_lib    ""
	set pot_focolare_lib "";#sim29
	set num_circuiti     "";#gac06
	set descr_comb       "";#gac06
	set descr_cost       "";#gac06
	set acquisti            "";#gac07
	set scorta_o_lett_iniz  "";#gac07
        set scorta_o_lett_fin   "";#gac07
	set acquisti2           "";#gac07
        set scorta_o_lett_iniz2 "";#gac07
        set scorta_o_lett_fin2  "";#gac07
	set um                  "";#gac07
	set elet_esercizio_1         "";#gac07
	set elet_esercizio_2         "";#gac07
	set elet_esercizio_3         "";#gac07
	set elet_esercizio_4         "";#gac07
	set elet_lettura_iniziale    "";#gac07
	set elet_lettura_finale      "";#gac07
	set elet_consumo_totale      "";#gac07
	set elet_lettura_iniziale_2  "";#gac07
	set elet_lettura_finale_2    "";#gac07
	set elet_consumo_totale_2    "";#gac07
    } else {
...
#sim29  element set_properties $form_name pot_utile_lib    -value $pot_utile_lib;#nic03
	...
	element set_properties $form_name num_circuiti     -value $num_circuiti;#gac06
	if {$coimtgen(regione) eq "MARCHE"} {#gac06 if e suo contenuto
	    element set_properties $form_name descr_cost       -value $descr_cost;#gac06
	    element set_properties $form_name descr_comb       -value $descr_comb;#gac06
	}

    }

    #gac07 aggiunta variabile unità di misura che viene valorrizzata a seconda del tipo di combustibile
	...
	#sim07	if {[db_0or1row sel_tari ""] == 0} {
	#sim07	    set tariffa ""
	#sim07	}
...
#	set flag_pagato ""
	...
	element set_properties $form_name cod_tprc         -value $cod_tprc;#Msim28
...
#sim28	element set_properties $form_name potenza          -value $pot_utile_nom;#nic03
	...
	if {$coimtgen(regione) ne "MARCHE"} {
	    ...
	    #gac06 Nel documento inviato da regione marche viene spiegato che non essendoci la norma UNI di riferimento il campo
	    #gac06 controllo del rendimento di combustione deve essere impostato di default "non effettuato" e non deve essere possibile
	...
	element set_properties $form_name cod_impianto     -value $cod_impianto
	element set_properties $form_name data_controllo   -value $data_controllo
        element set_properties $form_name data_ultima_manu -value $data_ultima_manu;#gac05
...
#       element set_properties $form_name rct_tratt_in_acs   -value $rct_tratt_in_acs
	...
	element set_properties $form_name fr_coibentazione -value $fr_coibentazione
       ...
       #freddo fine
	element set_properties $form_name acquisti         -value $acquisti ;#gac07
	element set_properties $form_name scorta_o_lett_iniz -value $scorta_o_lett_iniz ;#gac07
        element set_properties $form_name scorta_o_lett_fin  -value $scorta_o_lett_fin  ;#gac07
	element set_properties $form_name acquisti2         -value $acquisti2 ;#gac07
        element set_properties $form_name scorta_o_lett_iniz2 -value $scorta_o_lett_iniz2 ;#gac07
        element set_properties $form_name scorta_o_lett_fin2  -value $scorta_o_lett_fin2  ;#gac07
	element set_properties $form_name elet_esercizio_1        -value $elet_esercizio_1        ;#gac07
	element set_properties $form_name elet_esercizio_2        -value $elet_esercizio_2        ;#gac07
	element set_properties $form_name elet_esercizio_3        -value $elet_esercizio_3        ;#gac07
	element set_properties $form_name elet_esercizio_4        -value $elet_esercizio_4        ;#gac07
	element set_properties $form_name elet_lettura_iniziale   -value $elet_lettura_iniziale   ;#gac07
	element set_properties $form_name elet_lettura_finale     -value $elet_lettura_finale     ;#gac07
	element set_properties $form_name elet_consumo_totale     -value $elet_consumo_totale     ;#gac07
	element set_properties $form_name elet_lettura_iniziale_2 -value $elet_lettura_iniziale_2 ;#gac07
	element set_properties $form_name elet_lettura_finale_2   -value $elet_lettura_finale_2   ;#gac07
	element set_properties $form_name elet_consumo_totale_2   -value $elet_consumo_totale_2   ;#gac07
    ...
    if {$funzione eq "I"} {#gac05 
	set flag_pagato_dimp "";#gac05
	element set_properties $form_name flag_pagato -value $flag_pagato_dimp
    }


    db_0or1row q "select cod_tpco
                        from coimgend
                       where gen_prog =:gen_prog
                         and cod_impianto=:cod_impianto"
    
    set fr_assorb_recupero    "No"
    set fr_assorb_fiamma      "No"
    set fr_ciclo_compressione "No"
    
    switch $cod_tpco {
	1 {set fr_assorb_fiamma "Sì"}
	2 {set fr_ciclo_compressione "Sì"}
	3 {set fr_assorb_recupero "Sì"}
	4 {set fr_ciclo_compressione "Sì"}
    }
...
if {[form is_valid $form_name]} {
    ...
    set data_ultima_manu [string trim [element::get_value $form_name data_ultima_manu]];#gac05
    ...
    if {$coimtgen(regione) ne "MARCHE"} {#gac05 aggiunta if perche regione marche vuole che che flag pagato non venga selezionato in automatico ma deve essere inserito dall'utente
...
#    set rct_tratt_in_acs         [string trim [element::get_value $form_name rct_tratt_in_acs]]
    ...
    set rct_lib_uso_man_comp       [string trim [element::get_value $form_name rct_lib_uso_man_comp]]
    set scorta_o_lett_iniz      [string trim [element::get_value $form_name scorta_o_lett_iniz     ]];#gac07
    set scorta_o_lett_fin       [string trim [element::get_value $form_name scorta_o_lett_fin      ]];#gac07
    set acquisti                [string trim [element::get_value $form_name acquisti               ]];#gac07
    set acquisti2               [string trim [element::get_value $form_name acquisti2              ]];#gac07
    set scorta_o_lett_iniz2     [string trim [element::get_value $form_name scorta_o_lett_iniz2    ]];#gac07
    set scorta_o_lett_fin2      [string trim [element::get_value $form_name scorta_o_lett_fin2     ]];#gac07
    set elet_esercizio_1        [string trim [element::get_value $form_name elet_esercizio_1       ]];#gac07
    set elet_esercizio_2        [string trim [element::get_value $form_name elet_esercizio_2       ]];#gac07
    set elet_esercizio_3        [string trim [element::get_value $form_name elet_esercizio_3       ]];#gac07
    set elet_esercizio_4        [string trim [element::get_value $form_name elet_esercizio_4       ]];#gac07
    set elet_lettura_iniziale   [string trim [element::get_value $form_name elet_lettura_iniziale  ]];#gac07
    set elet_lettura_finale     [string trim [element::get_value $form_name elet_lettura_finale    ]];#gac07
    set elet_consumo_totale     [string trim [element::get_value $form_name elet_consumo_totale    ]];#gac07
    set elet_lettura_iniziale_2 [string trim [element::get_value $form_name elet_lettura_iniziale_2]];#gac07
    set elet_lettura_finale_2   [string trim [element::get_value $form_name elet_lettura_finale_2  ]];#gac07
    set elet_consumo_totale_2   [string trim [element::get_value $form_name elet_consumo_totale_2  ]];#gac07
	...
	set num_circuiti     [element::get_value $form_name num_circuiti];#gac06

	if {$coimtgen(regione) eq "MARCHE"} {#gac06 if e suo contenuto
	    set descr_cost [element::get_value $form_name descr_cost];#gac06
	    set descr_comb [element::get_value $form_name descr_comb];#gac06
	}
 ...
 set pot_focolare_nom [element::get_value $form_name pot_focolare_nom]
    #sim29    set pot_utile_lib [element::get_value $form_name pot_utile_lib];#nic03
...
#    element set_properties $form_name rct_tratt_in_acs   -value $rct_tratt_in_acs
	...
	#Msim28 aggiunto condizione su $changed_field eq "cod_tprc"
	if {$changed_field eq "flag_status" || $changed_field eq "cod_tprc" || $changed_field eq "flag_pagato"} {;#sim15 if e suo contenuto
	...
	if {$coimtgen(regione) eq "MARCHE"} {#gac05
	    ...
	    if {[string equal $data_ultima_manu ""]} {#gac05 aggiunta if else e loro contenuto
		...
		if {$data_ultima_manu == 0} {
		    ...
		    incr error_num
		} else {#gac05 aggiunto else e suo contenuto
	...
	if {$cont_rend == "S"} {
	...
	} elseif {$cont_rend eq "N"} {
	    if {$fr_surrisc ne "" ||
		$fr_sottoraff ne "" ||
		$fr_tcondens  ne "" ||
		$fr_tevapor ne "" ||
		$fr_t_ing_lato_est ne "" ||
		$fr_t_usc_lato_est ne "" ||
		$fr_t_ing_lato_ute ne "" ||
		...
		if {$fr_surrisc_prfumi($conta_prfumi) ne "" ||
		    $fr_sottoraff_prfumi($conta_prfumi) ne "" ||
		    $fr_tcondens_prfumi($conta_prfumi) ne "" ||
		    $fr_tevapor_prfumi($conta_prfumi) ne "" ||
		    $fr_t_ing_lato_est_prfumi($conta_prfumi) ne "" ||
		    $fr_t_usc_lato_est_prfumi($conta_prfumi) ne "" ||
		    $fr_t_ing_lato_ute_prfumi($conta_prfumi) ne "" ||
...
#sim39	if {$cont_rend ne "N"} {#gac06 aggiunto solo la if
	...
	if {$flag_mod_gend == "S"} {
	    if {$coimtgen(regione) ne "MARCHE"} {#gac06 aggiunta if non il suo contenuto
		...
		if {![string equal $data_insta ""]} {#sim33
		    set data_insta [iter_check_date $data_insta]
		};#sim33
		
		if {![string equal $data_costruz_gen ""]} {#sim33
		    set data_costruz_gen [iter_check_date $data_costruz_gen];#sim32
		};#sim33
	    }
	    if {$costruttore == ""} {
	...
	#gac08 essendo un campo da far vedere in sola visualizzazione non scommento nic03 perchè questi controlli sarebbero inutili
...
#sim29	if {![string equal $pot_utile_lib ""]} {#nic03: aggiunta questa if ed il suo contenuto
#sim29	    set pot_utile_lib [iter_check_num $pot_utile_lib 2]
#sim29	    if {[string equal $pot_utile_lib "0.00"]} {
#sim29		element::set_error $form_name pot_utile_lib "Deve essere diverso da 0,00"
#sim29		incr error_num
#sim29	    }
#sim29	    if {$pot_utile_lib == "Error"} {
#sim29		element::set_error $form_name pot_utile_lib "Deve essere numerico e pu&ograve; avere al massimo 2 decimali"
#sim29		incr error_num	    
#sim29	    }
#sim29	}

	#sim essendo un campo readonly non ha senso che venga fatto il controllo per la regione.
	#sim Inoltre per le marche può essere 0
	if {$coimtgen(regione) ne "MARCHE"} {
	...
	if {!($coimtgen(regione) in [list "MARCHE" "FRIULI-VENEZIA GIULIA"])} {#gac08 aggiunta if
	    ...
	    #gac08 Per Regione Marche il campo è in readonly e viene estratto direttamente dalla select.
	    #gac08 Essendo editato come stringa devo però fare la iter_check_num per avere il formato numerico in modo che
	    #gac08 la insert non vada in errore.
	    if {![string equal $potenza ""]} {#sim38 aggiunta if ma non suo contenuto
		set potenza [iter_check_num $potenza 2]
	    }
	}	    
	...
	db_1row query "select greatest(potenza, potenza_utile) as potenza_impianto
                         from coimaimp
                        where cod_impianto = :cod_impianto";#rom08
	...
	if {![string equal $riferimento_pag ""] && $tipologia_costo == "BO"} {
		...
		element::set_error $form_name riferimento_pag "Bollino già presente in altri allegati";#gac01
		incr error_num;#gac01
	    } else {#nic04
		set dbn_ente_collegato "";#nic04
		if {$coimtgen(ente) eq "PUD"} {#nic04
		    set dbn_ente_collegato "iterprgo";#nic04
		} elseif {$coimtgen(ente) eq "PGO"} {#nic04
		    set dbn_ente_collegato "iterprud";#nic04
		};#nic04

		if {$coimtgen(ente) eq "PAN"} {;#sim18 if e suo contenuto
		    set dbn_ente_collegato "itercman"
		} elseif {$coimtgen(ente) eq "CANCONA"} {;#sim18 if e suo contenuto 
		    set dbn_ente_collegato "iterpran"
		}

		if {$dbn_ente_collegato ne ""} {#nic04
		    db_1row -dbn $dbn_ente_collegato sel_dimp_check_riferimento_pag "";#nic04
		    if {$count_riferimento_pag > 0} {#nic04
			append note_todo_boll "Il bollino applicato sull'allegato e' gia' stato applicato precedentemente su un altro allegato inserito sull'altro ente\n";#nic04
		    };#nic04
		};#nic04
	    };#nic04
	    ...
	    if {$coimtgen(ente) eq "CPESARO"} { #gac02 aggiunta if
	    ...
	    } elseif {$coimtgen(ente) eq "CFANO" || $coimtgen(ente) eq "CJESI" || $coimtgen(ente) eq "CSENIGALLIA"} {;#gac02 else e suo contenuto
	    ...
	    if {$flag_boll_compreso == "f"} {
		if {$coimtgen(ente) eq "PLI"} {#sim05 Uniformato al coimdimp-rct-gest
		    element::set_error $form_name riferimento_pag "Il bollino non e' stato rilasciato al manutentore che ha compilato il modulo"
		    incr error_num
		} else {
		    # Caso standard
		    #gac01 append note_todo_boll "Il bollino applicato sull'allegato non e' stato rilasciato al manutentore che ha compilato il modulo"
		     element::set_error $form_name riferimento_pag "Il bollino applicato sulla Dichiarazione non e' stato rilasciato al manutentore che ha compilato il modulo";#gac01
		     incr error_num;#gac01
		}
	    }
	...
	if {![string equal $data_prox_manut ""]} {#gab01: aggiunta if ed il suo contenuto
            ...
            #sim36 if {$coimtgen(ente) eq "CANCONA" || $coimtgen(ente) eq "PAN" || $coimtgen(ente) eq "PFI"} {
	...
	if {$coimtgen(regione) in [list "CALABRIA" "TOSCANA" "FRIULI-VENEZIA GIULIA" "CAMPANIA"] && $coimtgen(ente) ni "CBENEVENTO PNA"} {#sim02: Aggiunta if e suo contenuto	   
		...
		if {$potenza < 100.00 && $flag_impianto_vecchio==0 && $locale eq "E"} {;#4 anni
		    set data_scadenza_autocert [db_string query "select :data_controllo::date + interval '4 year'" -default ""]
		} else {;#2 anno
		    set data_scadenza_autocert [db_string query "select :data_controllo::date + interval '2 year'" -default ""]
		}
...
#	}
    ...
    if {$coimtgen(regione) eq "MARCHE"} {#rom10 if e contenuto
	#Se sono un installatore e cerco di inserire l'RCEE posso farlo solo per alcune motivazioni.
	#Se l'istallatore figura anche come manutentore dell'impianto allora non lo devo bloccare.
	if {[db_0or1row q "select 1 
                             from coimaimp 
                            where cod_impianto = :cod_impianto
                              and cod_installatore = :cod_manu
                              and cod_manutentore != :cod_manu"] &&
	    !($cod_tprc in [list "PRIMMES" "SOSTGEN" "RISTRUT"])} {
    ...
    #gac07 aggiunto controlli su campi consumi ed elettricità
    ...
    #gac07 aggiunti controlli e calcolo consumo anno automatico nel caso in cui consumo_annuo o consumo_annuo2 siano vuoti
	...
	if {$is_warning_p eq "f"} {#gac05 aggiunto if e suo contenuto
	...
	if {$coimtgen(regione) eq "MARCHE"} {#gac05
		...
		#rom06element::set_error $form_name err_rcee "<font color=blue>ATTENZIONE: sono presenti degli errori nella pagina. Cliccando di nuovo su \"Conferma inserimento\", il REE verrà comunque inviato ma sarà sottoposto ad accertamento documentale in via prioritaria.<font>"
		element::set_error $form_name err_rcee "<font color=blue>ATTENZIONE: sono presenti degli errori nella pagina. Cliccando di nuovo su \"Conferma inserimento\", il REE verrà comunque inviato.<font>";#rom06
	    } else {
	...
	if {$flag_portafoglio == "T"
		...
		if {![string equal $cod_manu ""]} {
		    set url "lotto/itermove?iter_code=$cod_manu&body_id=1&tran_type_id=2&payment_type=1&payment_date=$oggi&reference=$reference&description=pagamento&amount=$coto_wallet&ente_portafoglio=$ente_portafoglio"
		    ...
		    set url "lotto/itermove?iter_code=$cod_manutentore&body_id=1&tran_type_id=2&payment_type=1&payment_date=$oggi&reference=$reference&description=pagamento&amount=$coto_wallet&ente_portafoglio=$ente_portafoglio"
    ...
    if {$fr_assorb_fiamma eq "Sì"} {
	set fr_assorb_fiamma "S"
    } else {
	set fr_assorb_fiamma "N"
    }

    if {$fr_assorb_recupero eq "Sì"} {
	set fr_assorb_recupero "S"
    } else {
	set fr_assorb_recupero "N"
    }

    if {$fr_ciclo_compressione eq "Sì"} {
	set fr_ciclo_compressione "S"
    } else {
	set fr_ciclo_compressione "N"
    }
