...
if {[form is_valid $form_name]} {
	...
	if {$cod_opmanu_new eq ""} {
	    #Per la provincia di Caserta deve essere facoltativo
	...
	if {$coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO"} {
	    # Per Udine e Gorizia e non va fatto il controllo sul codice fiscale ---tolto su Caserta il 16 11 2019
	} else {
	    # Caso Standard
	    if {$flag_errore_data_controllo == "f" && $data_controllo >= "20190101"} {
	...
	if {$funzione eq "I"} {
		...
		||  $coimtgen(ente) eq "CRIMINI"
		    ...
		    set oggi50 [clock format [clock scan "$data_controllo +60   days"] -format "%Y%m%d"];#Sandro 28/07/2014
	...
	db_1row query "select potenza as potenza_impianto
                         from coimaimp
                        where cod_impianto = :cod_impianto" 
	...
	if {![string equal $riferimento_pag ""] && $tipologia_costo == "BO"} {
		...
		if {$prosegui eq "t" && $potenza_impianto >= 35.00} {
			...
			if {($riferimento_pag_prefix ne "F1" && $riferimento_pag ne "F2")
			...
			db_foreach sel_boll_manu "" {
			    if {$matricola_da <= $riferimento_pag_num && $matricola_a >= $riferimento_pag_num} {
				ns_log Notice "coimdimp-f-gest;$matricola_da <= $riferimento_pag_num && $matricola_a >= $riferimento_pag_num"
				set flag_boll_compreso "t"
			    }
			}
		    }
		...
		set anno_boll  [string range $riferimento_pag 0 4]
		if {$tipologia_costo == "BO"
		    && ($anno_boll != "2008/" && $anno_boll != "2009/" && $anno_boll != "2010/" && $anno_boll != "2011/" && $anno_boll != "2012/" && $anno_boll != "2013/"  &&  $anno_boll != "2014/" && $anno_boll != "2015/" && $anno_boll != "2017/" && $anno_boll != "2018/" && $anno_boll != "2019/" && $anno_boll != "2020/" && $anno_boll != "2021/" && $anno_boll != "2022/" && $anno_boll != "2023/")
		} {
		    element::set_error $form_name riferimento_pag "Incongruenza Bollino/Tipologia Costo   es. anno/xxxx"
		    incr error_num
		}
		...
		set bol_nudo [iter_check_num [string range $riferimento_pag 5 [string length $riferimento_pag]]]
		set bol_nudo [iter_check_num $bol_nudo 0]
		...
		db_foreach sel_boll_manu "" {
		    ...
		    set mat_da [string range $matricola_da 5 [string length $matricola_da]]
		    set mat_a [string range $matricola_a 5 [string length $matricola_a]]
		    set mat_da [iter_check_num $mat_da 0]
		    set mat_a [iter_check_num $mat_a 0]
		    #set matricola_da [iter_check_num $matricola_da 0]
		    #set matricola_a [iter_check_num $matricola_a 0]
		    #set riferimento_pag [iter_check_num [string range $riferimento_pag 5 [string length $riferimento_pag]] 0]
		    
		    if {$mat_da != 0 && $mat_a != 0 && $bol_nudo != "Error"} {
		...
		if {$flag_boll_compreso == "f"} {
		    element::set_error $form_name riferimento_pag "Il Bollino non e' stato rilasciato al manutentore che ha compilato il modulo oppure Incongruenza Bollino es. anno/xxxxx "
		    incr error_num
		}
		...
		if {$flag_boll_compreso == "f"} {
		    append note_todo_boll "Il bollino applicato sulla Dichiarazione non e' stato rilasciato al manutentore che ha compilato il modulo es. anno/xxxx"
		}
	    }
	...
	if {![string equal $data_scad_pagamento ""]} {
        ...
        if {$flag_ente eq "C" && $coimtgen(denom_comune) eq "RIMINI"}  {
		...
		&&  $sw_pot_focolare_nom_ok == "t"
		    ...
		    if {$pot_focolare_nom < 35.00 && ![db_0or1row q "select 1 where :data_controllo >= '20190101'"]} {
