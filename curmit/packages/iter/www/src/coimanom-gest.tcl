ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimanom"
...
if {$funzione == "I"} {
    ...
    -optional \
    -options [iter_selbox_from_table coimtano cod_tano "cod_tano||' - '||descr_breve"]
} else {
...
if {[form is_valid $form_name]} {
  # form valido dal punto di vista del templating system
	...
	if {$funzione == "I"} {
		...
		element::set_error $form_name cod_tanom "Inserire Cod. tipo anomalia"
		incr error_num
	    }
	}
...
#		set dml_cimp [db_map upd_cimp]
    ...
    if {$funzione == "D"} {
	set flag_pericolosita "F"
	db_foreach sel_tano_scatenante "" {
	    if {$flag_scatenante == "T"} {
		set flag_pericolosita "T"
	    } 
	} if_no_rows {
	    set flag_pericolsita "F"
	}
	switch $flag_origine {
	...
	#	set cod_cimp_dimp $cod_cimp
        ...
        M {set dml_sql [db_map upd_anom]
	    if {[db_0or1row check_todo ""] == 1} {
		set dml_sql2 [db_map upd_todo]
	    }
	}
        D {set dml_sql [db_map del_anom]
	    if {[db_0or1row check_todo ""] == 1} {
  ...
  # dopo l'inserimento posiziono la lista sul record inserito
    ...
    switch $funzione {
        M {set return_url   "coimanom-gest?funzione=V&$link_gest"}
        D {set return_url   "coimanom-list?$link_list"}
        I {set return_url   "coimanom-gest?funzione=V&$link_gest"}
        V {set return_url   "coimanom-list?$link_list"}
    }
