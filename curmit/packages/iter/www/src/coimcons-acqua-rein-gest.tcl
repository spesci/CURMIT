ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimcons_acqua_rein"
    @author          Romitti Luca
...
set link_gest [export_url_vars cons_acqua_rein_id cod_impianto nome_funz nome_funz_caller url_list_aimp url_aimp extra_par caller]
...
set link_list_script {[export_url_vars cons_acqua_rein_id cod_impianto caller nome_funz_caller nome_funz url_list_aimp url_aimp]&[iter_set_url_vars $extra_par]}
...
set form_name    "coimcons_acqua_rein"
...
element create $form_name cons_acqua_rein_id -widget hidden -datatype text -optional
...
if {[form is_request $form_name]} {
    ...
    if {$funzione == "I"} {
	db_1row query "select coalesce(max(cons_acqua_rein_id) + 1, '1') as cons_acqua_rein_id from coimcons_acqua_rein"
	
	element set_properties $form_name cons_acqua_rein_id -value $cons_acqua_rein_id
	...
	if {[db_0or1row query "
	       ...
	       from coimcons_acqua_rein a
	      where a.cons_acqua_rein_id = :cons_acqua_rein_id
        "] == 0
	} {
...
if {[form is_valid $form_name]} {
    ...
    set cons_acqua_rein_id   [element::get_value $form_name cons_acqua_rein_id]
		...
		M {
		    db_dml query "
                    update coimcons_acqua_rein
                     ...
                     where cons_acqua_rein_id = :cons_acqua_rein_id"
		}
		D {
		    db_dml query "
                    delete
    ...
    set link_list      [subst $link_list_script]
    set link_gest      [export_url_vars cod_impianto cons_acqua_rein_id nome_funz nome_funz_caller url_list_aimp url_aimp ]
    switch $funzione {
