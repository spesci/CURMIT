ad_page_contract {
    Add              form per la tabella "coimbatc"
                     Lancio batch di caricamento RCEE di tipo 1 da file esterno
    @author          Simone Pesci (clonato da coimcari-modelli-g-gest)
    ...
    @cvs-id          coimcari-rcee-tipo-1-gest.tcl
...
set titolo              "Caricamento RCEE di tipo 1"
...
if {[form is_valid $form_name]} {
  ...
  # Lancio la query di manipolazione dati contenute in dml_sql
    ...
    switch $funzione {
        I {set return_url   "coimcari-rcee-tipo-1-gest?funzione=V&$link_gest"}
        V {set return_url   ""}
    }
