...
set titolo              "Caricamento rapporti di verifica"
switch $funzione {
...
if {[form is_request $form_name]} {
	...
	element set_properties $form_name current_time  -value $current_time
      ...
      # leggo riga
        if {[db_0or1row query "select iter_edit_data(dat_prev) as dat_prev
                                    , iter_edit_time(ora_prev) as ora_prev
                                    , par
                                 from coimbatc
                                where cod_batc = :cod_batc"] == 0} {
...
if {[form is_valid $form_name]} {
  ...
  # controlli standard su numeri e date, per Ins ed Upd
    ...
    if {$funzione == "I"} {
	# set cod_batc
	db_1row query "select nextval('coimbatc_s') as cod_batc"
	set flg_stat       "A"
	set cod_uten_sch   $id_utente
	set nom_prog       "iter_controlli_cari"
...
ns_log notice "prova dobo nome file $file_name"
...
ns_log notice "prova dobo salvato nome file $file_name"
...
ns_log notice "prova dobo inserimento in batch"
    #
...
"
    ...
    switch $funzione {
        I {set return_url   "coimcari-controlli?funzione=V&$link_gest"}
        V {set return_url   ""}
    }
    
    ad_returnredirect $return_url
#     ad_returnredirect "iter-controlli-cari-procs?file_name=$file_name"
