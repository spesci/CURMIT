ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimaltr_gend_aimp"
...
set link_gest [export_url_vars cod_altr_gend_aimp cod_impianto nome_funz nome_funz_caller url_list_aimp url_aimp extra_par caller]
...
set link_list_script {[export_url_vars cod_altr_gend_aimp cod_impianto caller nome_funz_caller nome_funz url_list_aimp url_aimp]&[iter_set_url_vars $extra_par]}
set link_list        [subst $link_list_script]

set current_date     [iter_set_sysdate]

set titolo              "Altro Generatore"
switch $funzione {
...
set form_name    "coimaltr_gend_aimp"
...
element create $form_name num_ag \
    -label   "Numero" \
    -widget   text \
    -datatype text \
...
#rom02
element create $form_name num_ag_sostituente \
    -label "AG sostituente" \
    -widget text \
    -datatype text \
...
element create $form_name cod_altr_gend_aimp  -widget hidden -datatype text -optional
...
if {[form is_request $form_name]} {    
	...
	# propongo il numero del nuovo generatore con il max + 1
        db_1row query "
        select coalesce(max(num_ag),0) + 1 as num_ag 
          from coimaltr_gend_aimp
         where cod_impianto = :cod_impianto"
        
        element set_properties $form_name num_ag -value $num_ag
      
	db_1row query "select nextval('coimaltr_gend_aimp_s') as cod_altr_gend_aimp"
	
	element set_properties $form_name cod_altr_gend_aimp -value $cod_altr_gend_aimp
	...
	if {[db_0or1row query "            
          ...
          left join coimcost b
              ...
              where a.cod_altr_gend_aimp = :cod_altr_gend_aimp
        "] == 0
        } {
	...
	element set_properties $form_name num_ag_sostituente  -value $num_ag_sostituente;#rom02
...
if {[form is_valid $form_name]} {
    ...
    set cod_altr_gend_aimp   [element::get_value $form_name cod_altr_gend_aimp]
    set cod_impianto         [element::get_value $form_name cod_impianto]
    set num_ag               [element::get_value $form_name num_ag]
    ...
    set num_ag_sostituente   [element::get_value $form_name num_ag_sostituente];#rom02
    ...
    set potenza_utile        [element::get_value $form_name potenza_utile]
	...
	if {$potenza_utile ne ""} {
            ...
            if {$potenza_utile == "Error"} {
                element::set_error $form_name potenza_utile "Deve essere numerico, max 2 dec"
                incr error_num
            } elseif {$potenza_utile < 0.01} {
	...
	if {![string is space $num_ag_sostituente]} {#rom02 if e contenuto
	    set num_ag_sostituente [iter_check_num $num_ag_sostituente 0]
	    if {$num_ag_sostituente == "Error" || [db_0or1row q "select 1 
                                                                   from coimaltr_gend_aimp
                                                                  where num_ag = :num_ag 
                                                                    and cod_impianto = :cod_impianto
                                                                    and num_ag = :num_ag_sostituente
                                                                     or :num_ag = :num_ag_sostituente
                                                                     or :num_ag_sostituente = 0 limit 1"] == 1} {
		element::set_error $form_name num_ag_sostituente "Deve essere un numero intero diverso da 0 e da se stesso"
		incr error_num
	    } 
	};#rom02
	if { [string is space $num_ag_sostituente] && $flag_sostituito eq "t"} {#rom02 if, elseif e contenuto
	    element::set_error $form_name num_ag_sostituente "Inserire quale sarà l'AG sostituente"
	    incr error_num
	} elseif {![string is space $num_ag_sostituente] && $flag_sostituito ne "t"} {
	    element::set_error $form_name num_ag_sostituente "Inserire solo se si sostituisce l'AG"
	    incr error_num
	};#rom02
		    ...
		    db_dml query "
                    update coimaltr_gend_aimp
                       set data_installaz   = :data_installaz
                         ...
                         , num_ag_sostituente = :num_ag_sostituente
                     where cod_altr_gend_aimp = :cod_altr_gend_aimp"
		}
		D {
		    db_dml query "
                    delete 
    ...
    set link_list      [subst $link_list_script]
    set link_gest      [export_url_vars cod_impianto cod_altr_gend_aimp nome_funz nome_funz_caller url_list_aimp url_aimp ]
    switch $funzione {
