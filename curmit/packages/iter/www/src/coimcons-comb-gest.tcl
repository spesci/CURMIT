ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimcons_comb"
    @author          Romitti Luca
...
set link_gest [export_url_vars cons_comb_id cod_impianto nome_funz nome_funz_caller url_list_aimp url_aimp extra_par caller]
...
set link_list_script {[export_url_vars cons_comb_id cod_impianto caller nome_funz_caller nome_funz url_list_aimp url_aimp]&[iter_set_url_vars $extra_par]}
...
element create $form_name esercizio \
...
element create $form_name cons_comb_id -widget hidden -datatype text -optional
...
if {[form is_request $form_name]} {
    ...
    if {$funzione == "I"} {
	db_1row query "select coalesce(max(cons_comb_id) + 1, '1') as cons_comb_id from coimcons_comb"
	
	element set_properties $form_name cons_comb_id -value $cons_comb_id
	...
	if {[db_0or1row query "
	       ...
	       from coimcons_comb a
	      where a.cons_comb_id = :cons_comb_id
        "] == 0
	} {
	...
	element set_properties $form_name cons_comb_id -value $cons_comb_id
	element set_properties $form_name cod_impianto        -value $cod_impianto
	element set_properties $form_name esercizio          -value $esercizio
	element set_properties $form_name acquisti          -value $acquisti
...
if {[form is_valid $form_name]} {
    ...
    set cons_comb_id   [element::get_value $form_name cons_comb_id]
    set cod_impianto         [element::get_value $form_name cod_impianto]
    set esercizio           [element::get_value $form_name esercizio]
    set acquisti           [element::get_value $form_name acquisti]
	...
	if {$funzione == "I" || $funzione == "M"} {   
	    ...
	    if {[string is space $esercizio]} {
		element::set_error $form_name esercizio "Inserire l'esercizio"
		incr error_num
	    } 
	    if {![string equal $esercizio ""]} {
		set esercizio [iter_check_num $esercizio 0]
		if {$esercizio == "Error"} {
		    element::set_error $form_name esercizio "Esercizio deve essere numerico"
		    incr error_num
		}
	    }
	    if {![string equal $acquisti ""]} {
		set acquisti [iter_check_num $acquisti 0]
		if {$acquisti == "Error"} {
		    element::set_error $form_name acquisti "acquisti deve essere numerico"
		    incr error_num
		}
	    }
	...
	db_transaction {
		...
		M {
		    db_dml query "
                    update coimcons_comb
                       set esercizio            = :esercizio
                         , acquisti             = :acquisti
                     ...
                     where cons_comb_id = :cons_comb_id"
		}
		D {
		    db_dml query "
                    delete
    ...
    set link_list      [subst $link_list_script]
    set link_gest      [export_url_vars cod_impianto cons_comb_id nome_funz nome_funz_caller url_list_aimp url_aimp ]
    switch $funzione {
