ad_page_contract {
    Add              form per la tabella "coimbatc"
                     Lancio batch di caricamento RCEE di tipo 1 da file xml esterno
    @author          Luca Romitti (clonato da coimcari-rcee-tipo-1-gest)
    ...
    @cvs-id          coimcari-rcee-xml-tipo-1-gest.tcl
    
    USER  DATA       MODIFICHE
    ===== ========== ============================================================================================
    rom00 23/04/2024 Programma copiato da coimcari-rcee-xml
...
set nom       "Cari. forniture Distr. XML"
...
if {[form is_valid $form_name]} {
  ...
  # controlli standard su numeri e date, per Ins ed Upd
	...
	if {[string equal $file_name ""]} {
	    ...
	    if {$extension != ".xml"} {
		element::set_error $form_name file_name "Il file non ha estensione XML."
		incr error_num
	    } else {
	...
	set nom_prog "iter_cari_distr_forn_xml"
	set par            ""
  ...
  # Lancio la query di manipolazione dati contenute in dml_sql
    ...
    switch $funzione {
        I {set return_url   "coimcari-distr-forn-xml-gest?funzione=V&$link_gest"}
        V {set return_url   ""}
    }
