ad_page_contract {
    ...
    mat02            Adesso la DFM è obbligatoria e la query che fa comparire il messaggio non funziona correttamente, Sandro
    mat02            ha detto di non perderci troppo tempo e che adesso il messaggio non serve più, quindi metto 1==0 nella
    mat02            condizione.
    
    ric03 03/10/2025 Aggiunta lettura storico ditta di manutenzione (Punto 4 MEV Regione Marche).
    ...
    but02 14/05/2024 Solo per le Marche ho aggiunto un messaggio d'attenzione se  il DFM non esiste.
    
    rom14 04/12/2023 Rieti ha chiesto di avere come codice bollino il valore dell'id cod_dimp come gia' fatto per Napoli e Palermo.

    ric02 16/11/2023 Settata scadenza per Provincia di Rieti.
    ...
    rom12 19/10/2023 Sandro ha chiesto di riportare la modifica di rom07 fatta per Palermo anche per Napoli:
    rom12            il codice bollino sara' valorizzato con il valore dell'id cod_dimp.

    but01 21/07/2023 Aggiunto la classe ah-jquery-date ai campi:data_prox_manut, data_scad_pagamento , data_scadenza_autocert 
    but01            , data_arrivo_ente, data_ut_int, data_insta,data_controllo, data_ultima_manu, data_prot. 

    rom12 19/07/2023 Udine e Gorizia non facevano il controllo sul codice fiscale, ora la if e' su tutta regione Fiuli.

    rom11 02/03/2023 Committata vecchia modifica fatta per Regione Marche: in certi casi non uscivano i messaggi di errore.

    rom10 01/03/2023 Settata correttamente la scadenza per Regione Marche, se la potenza e' maggiore di 10 kW (cioe' sempre)
    rom10            la scadenza e' sempre di 4 anni.
    ...
    rom08 01/08/2022 Modifiche per allineamento enti di Ucit al nuovo csv:
    rom08            Le particolarita' e le modifiche dei vari enti di UCIT le vado a mettere
    rom08            dentro ad un'unica condizione sulla Regione Friuli.
    rom08            Per il teleriscaldamento gli enti di Ucit non devono avere particolarità
    rom08            nella scadenze ma usare il parametro standard (48 mesi)
    rom08            Per il momento non tocco neanche le personalizzazioni che riguardano solo
    rom08            le provincie di Udine e Gorizia perche' Sandro vuole cosi'.
    rom08            Sandro ha detto che gli impianti del Comune di Udine devono avere il
    rom08            costo 0 per il teleriscaldamento.
    
    rom07 06/12/2022 Implementazione per Palermo richiesta con mail "Implementazioni" di Sandro del 01/12/2022, riporto testo:
    ...
    rom07            Gli rcee sul secondo generatore sono esenti su Palermo. Per Palermo il flag_pagato va messo sempre a Si.
    rom07            Con Sandro si e' deciso di fare le if solo per Palemo e non per tutta la Regione Sicilia perche' la citta'
    rom07            metropolitana ha particolarita' proprie.
    
    rom06 20/12/2021 Modificate le scadenze settate da rom05 su Basilicata per nuova legge Regionale.

    gia01 08/11/2021 Commentati i controlli sulle anomalie.

    rom05 13/05/2021 Settate le scadenze per la Regione Basilicata.
    ...
    gac05 22/11/2019 Se non ho inserito tutti  i campi obbligatori posso entrare in sola visualizzazione

    rom03 28/02/2019 Aggiunto campo cont_rend su richiesta della Regione Marche.-

    gac04 13/02/2019 Su richiesta della regione marche ho tolto l'opzione NC per i campi della sezione B.
    ...
    rom02 16/08/2018 Tolta obbligatorietà sui campi delle potenze per Ucit.
    ...
    sim47 19/04/2018 Corretto errore causato dal formato di data_insta
    ...
    rom01 02/02/2018 Modificata la variabile titolo da "RCEE Tipo 1" a "RCEE Tipo 3" .
...
if {$is_only_view} {#gac05 aggiunta if e suo contenuto
    set menu 0
    set funzione V
}
...
#rom09 spostatao piu' in alto iter_get_coimtgen
...
#set coimtgen(regione) "MARCHE!"
set err_rcee "";#gac02
set msg_rcee "";#gac02

if {$coimtgen(regione) eq "MARCHE"} {#rom11 Aggiunta if e il suo contenuto
    set nome_col_aimp_potenza potenza_utile
    set error_nome_col_potenza "pot_ter_nom_tot_max"

    set msg_dfm "";#but02
    set url_dfm "https://www.regione.marche.it/portals/0/Energia/ImpiantiTermici/procedura_telerisc_completa_genn_2022.pdf";#but02
    set msg_dfm "";#mat02
    if {$funzione eq "I" && 1==0} {#mat02 aggiunta condizione 1==0 #but02 Aggiunta if e suo contenuto
	if {![db_0or1row q "select 1
                             ...
                             limit 1"]} {
	    set msg_dfm "ATTENZIONE, Non hai inserito la DFM! Verifica se è obbligatoria o meno controllando <a href='$url_dfm'>QUI</a>." 
	} else {
	    set msg_dfm ""
	}
    }
    
} else {#rom11 Aggiunta else ma non il suo contenuto
...
set controllo_tariffa {#sim08
	...
	if {[form is_request $form_name] || 
	    ...
	    #sim47 in alcuni casi entra nel formato dd/mm/yyyy quindi per sicurezza ho messo questo if
	    if {[iter_check_date $data_insta] != 0} {#sim47 if e suo contenuto
		set data_insta_controllo [iter_check_date $data_insta]
    ...
    } elseif {$coimtgen(ente) eq "PPA" && $rcee_su_secondo_gen eq "t"} {#rom07 Aggiunta elseif e il suo contenuto
        set tariffa 0
        set esente "t"
    } else {
	set esente "f"
    ...
    if {$coimtgen(ente) eq "PUD" &&
        [db_0or1row q "select 1
                        ...
                        where cod_impianto = :cod_impianto
                          and cod_comune = '140'"] &&
        $data_controllo < "20210101"
    } {#rom08 aggiunta elseif e suo contenuto
        set tariffa 0
    }
...
set ast_tel_pot_obbl "<font color=red>*</font>";#rom0
set tel_pot_obbl "t";#rom02
...
#rom03 aggiunta condizione per la Regione Marche, per loro i campi delle potenze sono obbligatori in base al campo cont_rend
#rom08 Tolte condizioni $coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO" || $coimtgen(ente) eq "PTS" || $coimtgen(ente) eq "PPN"
#rom08 Al loro posto messa unica condizione su Regione Friuli.
if {$coimtgen(regione) in [list "MARCHE" "FRIULI-VENEZIA GIULIA"]} {#rom02 if e contenuto
    set ast_tel_pot_obbl ""
    set tel_pot_obbl     "f"
}
...
#rom01 modificato titolo da "RCEE Tipo 1" a "RCEE Tipo 3"
set titolo "RCEE tipo 3"
switch $funzione {
...
if {$flag_mod_gend == "S"} {
    if {$coimtgen(regione) ne "MARCHE"} {#gac03 if else e loro contenuto 
	...
	-options $cod_com
    # fine
#but01 Aggiunto la classe ah-jquery-date al campo data_insta.
...
#	-optional
    ...
    } else {#gac03 parte per regine marche
	...
	element create $form_name pot_focolare_nom -widget hidden -datatype text -optional;#gac03
...
#	-optional
    ...
    if {$coimtgen(regione) ne "MARCHE"} {#gac03 if else e contenuto di else
    element create $form_name costruttore \
	-label   "Costruttore" \
	-widget   text \
	-datatype text \
	-html    "size 15 maxlength 100 readonly {} class form_element" \
	-optional
#but0 Aggiunto la classe ah-jquery-date al campo data_insta.
	element create $form_name data_insta \
	-label   "Data installazione" \
	-widget   text \
	-datatype text \
	...
	element create $form_name data_insta -widget hidden -datatype text -optional
        element create $form_name pot_focolare_nom -widget hidden -datatype text -optional
	element create $form_name costruttore -widget hidden -datatype text -optional
	element create $form_name descr_cost \
	    -label   "descr_cost" \
	    -widget   text \
	    -datatype text \
...
if {$coimtgen(regione) eq "MARCHE"} {#gac04 if else e contenuto di if
...
#ale teleriscaldamento
element create $form_name tel_stato_coibent_idoneo_imp \
    -label   "tel_stato_coibent_idoneo_imp" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {N.C. C} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
element create $form_name tel_assenza_perdite_circ_idraulico \
    -label   "Assenza perdite del circuito idraulico" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {N.C. C} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
if {$coimtgen(regione) eq "MARCHE"} {
    element create $form_name tel_alimentazione_gen \
	-label   "Alimentazione" \
	-widget   text \
	-datatype text \
	-html    "readonly {} class form_element" \
	-optional
    element create $form_name tel_alimentazione -widget hidden -datatype text -optional
    
    element create $form_name tel_alimentazione_altro \
	-label   "Note altro alimentazione" \
	-widget   textarea \
	-datatype text \
    ...
    #ale teleriscaldamento
    element create $form_name tel_fluido_vett_term_uscita_gen \
	-label   "Fluido vettore termico in uscita" \
	-widget   text \
	-datatype text \
	-html    "readonly {} class form_element" \
	-optional
    element create $form_name tel_fluido_vett_term_uscita -widget hidden -datatype text -optional
	...
	-label   "Fluido vettore termico in uscita" \
	-widget   select \
	-options  {{{} {}} {"Acqua calda" A} {Vapore B} {Altro Z}} \
	-datatype text \
	-html    "$disabled_fld {} class form_element" \
	-optional
};#gac00
...
#ale teleriscaldamento
element create $form_name tel_potenza_compatibile_dati_prog \
    -label   "Potenza compatibile con i dati di progetto" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {N.C. C} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
element create $form_name tel_stato_coibent_idoneo_scamb \
    -label   "Stato delle coibentazioni idoneo" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {N.C. C} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
element create $form_name tel_disp_regolaz_controll_funzionanti \
    -label   "Dispositivi di regolazione e controllo funzionanti" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {N.C. C} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
#element create $form_name tel_assenza_trafil_valv_regolaz \
#    -label   "Assenza di trafilamenti sulla valvola di regolazione" \
#    -widget   select \
#    -options  {{S&igrave; S} {No N} {N.C. C} {{} {}}} \
#    -datatype text \
#    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
element create $form_name tel_temp_est \
...
#ale teleriscaldamento
element create $form_name tel_temp_mand_prim \
        -label   "Temperatura mandata Primario" \
        -widget   text \
        -datatype text \
        -html    "size 10 $readonly_fld {} class form_element" \
        -optional
...
#ale teleriscaldamento
element create $form_name tel_temp_rit_prim \
        -label   "Temperatura ritorno Primario" \
        -widget   text \
        -datatype text \
    -html    "size 10 $readonly_fld {} class form_element" \
        -optional
...
#ale teleriscaldamento
element create $form_name tel_potenza_termica \
...
#ale teleriscaldamento
element create $form_name tel_portata_fluido_prim \
        -label   "Portata fluido Primario" \
        -widget   text \
        -datatype text \
        -html    "size 10 $readonly_fld {} class form_element" \
        -optional
...
#ale teleriscaldamento
element create $form_name tel_temp_mand_sec \
        -label   "Temperatura mandata Secondario" \
        -widget   text \
        -datatype text \
    -html    "size 10 $readonly_fld {} class form_element" \
        -optional
...
#ale teleriscaldamento
element create $form_name tel_temp_rit_sec \
        -label   "Temperatura ritorno Secondario" \
        -widget   text \
        -datatype text \
        -html    "size 10 $readonly_fld {} class form_element" \
        -optional
...
#ale teleriscaldamento
element create $form_name tel_check_coerenza_paramentri \
...
#ale teleriscaldamento
element create $form_name tel_check_perdite_h2o \
    -label   "Verifica presenza perdite di acqua" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
#ale teleriscaldamento
element create $form_name tel_check_install_involucro \
    -label   "Installazione di adeguato involucro" \
    -widget   select \
    -options  {{S&igrave; S} {No N} {{} {}}} \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
...
element create $form_name data_utile_inter \
    ...
    -html    "size 10 maxlength 10 $readonly_fld {} class form_element" \
    -optional
#but01 Aggiunto la classe ah-jquery-date ai campi data_controllo, data_ultima_manu, data_prot.
...
#gac03 aggiunto per regione marche
if {$coimtgen(regione) eq "MARCHE"} {
...
element create $form_name costo \
    ...
    -optional
#but01 Aggiunto la classe ah-jquery-date al campo data_prox_manut.
...
if {$coimtgen(ente) eq "PPA"} {#rom07 Aggiunte if, else e loro contenuto
...
#but01 Aggiunto la classe ah-jquery-date al campo data_scadenza_autocert
...
element create $form_name num_autocert \
    ...
    -optional
#but01 Aggiunto la classe ah-jquery-date al campo data_arrivo_ente
...
#but01 aggiunto la classe ah-jquery-date al campo data_ut_int.
	...
	-widget   text \
	-datatype text \
	-html     "size 10 maxlength 10 $readonly_fld {} class form_element $jq_date $jq_date" \
	-optional 
}
...
if {$coimtgen(regione) eq "MARCHE"} {#rom03 aggiunte if, else e loro contenuto
...
if {[form is_request $form_name]} {
	...
	set cod_cind [db_string query "select max(cod_cind) from coimcind where stato = '1'" -default ""]  
		...
		# gen_prog e' gia' stato valorizzato con sel_gend_list
		    ...
		    set link_gend "flag_tracciato=R3&[export_url_vars cod_impianto last_cod_cimp nome_funz nome_funz_caller tabella cod_dimp extra_par caller url_aimp url_list_aimp]"
	...
	set flag_clima_invernale     "";#gac03
	set flag_prod_acqua_calda    "";#gac03

    } else {
	element set_properties $form_name costruttore      -value $costruttore
	if {$coimtgen(regione) eq "MARCHE"} {#gac03 aggiunta if
    ...
    if {$coimtgen(regione) eq "MARCHE"} {
	#se sono le marche vao a prendere i dati dell'alimentazione sul generatore
	db_1row q "select case tel_alimentazione
                              when 'A' then 'Acqua calda'
                              when 'B' then 'Acqua surriscaldata'
                              when 'C' then 'Vapore'
                              when 'Z' then 'Altro'
                              else ''  end as tel_alimentazione_gen_edit
                            , coalesce(tel_alimentazione,'') as tel_alimentazione
                            , case mod_funz 
                              when '0' then 'Non noto'
                              when '1' then 'Acqua calda'
                              when '2' then 'Aria'
                              when '3' then 'Altro'
                              when '4' then 'Acqua Surriscaldata'
                              when '5' then 'Vapore'
                              when '6' then 'Olio diatermico'
                              else '' end as tel_fluido_vett_term_uscita_gen_edit
                            , coalesce(mod_funz,'') as tel_fluido_vett_term_uscita_gen
                         from coimgend
                        where cod_impianto = :cod_impianto
                          and gen_prog     = :gen_prog"
	element set_properties $form_name tel_alimentazione_gen           -value $tel_alimentazione_gen_edit
	element set_properties $form_name tel_alimentazione               -value $tel_alimentazione
	element set_properties $form_name tel_fluido_vett_term_uscita_gen -value $tel_fluido_vett_term_uscita_gen_edit
	element set_properties $form_name tel_fluido_vett_term_uscita     -value $tel_fluido_vett_term_uscita_gen_edit
	...
	if {$flag_portafoglio == "T"} {
		...
		&& [db_0or1row q "select 1
                                   ...
                                   where cod_impianto = :cod_impianto
                                     and cod_comune    = '140'"]
		&& $data_controllo < "20210101"} {#rom08 aggiunta if e suo contenuto
	...
	if {$coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO"} {#nic04: aggiunta questa if ed il suo contenuto (copiandola da coimdimp-f-gest di Udine)
	    ...
	    ns_log Notice "coimdimp-r3-gest 1.1;potenza:$potenza|num_gend_attivi:$num_gend_attivi|num_dich_recedenti:$num_dich_recenti|cod_impianto:$cod_impianto|dt_recente:$dt_recente|"
...
#nic03	element set_properties $form_name potenza          -value $potenza_old_edit
	...
	set cod_man [iter_check_uten_manu $id_utente]
        #rom09 Aggiunta condizione su Regione Friuli
	...
	#ale01 teleriscaldamento
        element set_properties $form_name tel_stato_coibent_idoneo_imp          -value $tel_stato_coibent_idoneo_imp
        element set_properties $form_name tel_linee_eletr_idonee                -value $tel_linee_eletr_idonee
        element set_properties $form_name tel_assenza_perdite_circ_idraulico    -value $tel_assenza_perdite_circ_idraulico
	...
	if {$coimtgen(regione) ne "MARCHE"} {
	    ...
	    db_1row q "select case tel_alimentazione
                            ...
                            , coalesce(mod_funz,'') as tel_fluido_vett_term_uscita
                         from coimgend
                        where cod_impianto = :cod_impianto
                          and gen_prog     = :gen_prog"
	    element set_properties $form_name tel_alimentazione_gen           -value $tel_alimentazione_gen_edit
	...
	element set_properties $form_name tel_alimentazione_altro         -value $tel_alimentazione_altro
        element set_properties $form_name tel_fluido_altro                      -value $tel_fluido_altro        
	element set_properties $form_name tel_potenza_compatibile_dati_prog     -value $tel_potenza_compatibile_dati_prog
        element set_properties $form_name tel_stato_coibent_idoneo_scamb        -value $tel_stato_coibent_idoneo_scamb
        element set_properties $form_name tel_disp_regolaz_controll_funzionanti -value $tel_disp_regolaz_controll_funzionanti
#        element set_properties $form_name tel_assenza_trafil_valv_regolaz       -value $tel_assenza_trafil_valv_regolaz
        element set_properties $form_name tel_temp_est                          -value $tel_temp_est
        element set_properties $form_name tel_temp_mand_prim                    -value $tel_temp_mand_prim
        element set_properties $form_name tel_temp_rit_prim                     -value $tel_temp_rit_prim
        element set_properties $form_name tel_potenza_termica                   -value $tel_potenza_termica
        element set_properties $form_name tel_portata_fluido_prim               -value $tel_portata_fluido_prim
        element set_properties $form_name tel_temp_mand_sec                     -value $tel_temp_mand_sec
        element set_properties $form_name tel_temp_rit_sec                      -value $tel_temp_rit_sec
        element set_properties $form_name tel_check_coerenza_paramentri         -value $tel_check_coerenza_paramentri
        element set_properties $form_name tel_check_perdite_h2o                 -value $tel_check_perdite_h2o
        element set_properties $form_name tel_check_install_involucro           -value $tel_check_install_involucro
	# agg dob cind
	element set_properties $form_name cod_cind            -value $cod_cind
	element set_properties $form_name cont_rend         -value $cont_rend;#rom03
...
if {[form is_valid $form_name]} {
    ...
    set cont_rend        [string trim [element::get_value $form_name cont_rend]];#rom03
    ...
    } elseif {$coimtgen(ente) eq "PPA"} {#rom07 Aggiunta elseif e contenuto
        element set_properties $form_name flag_pagato -options {{"" ""} {S&igrave; S}}	
    ...
    #ale01 teleriscaldamento
    set tel_stato_coibent_idoneo_imp          [string trim [element::get_value $form_name tel_stato_coibent_idoneo_imp]]
    set tel_linee_eletr_idonee                [string trim [element::get_value $form_name tel_linee_eletr_idonee]]
    set tel_assenza_perdite_circ_idraulico    [string trim [element::get_value $form_name tel_assenza_perdite_circ_idraulico]]
    set tel_alimentazione                     [string trim [element::get_value $form_name tel_alimentazione]]
    set tel_fluido_vett_term_uscita           [string trim [element::get_value $form_name tel_fluido_vett_term_uscita]]
    set tel_alimentazione_altro               [string trim [element::get_value $form_name tel_alimentazione_altro]]
	...
	set tel_alimentazione $tel_alimentazione
	set tel_fluido_vett_term_uscita $tel_fluido_vett_term_uscita
	element set_properties $form_name tel_alimentazione_gen           -value $tel_alimentazione_gen_edit
	element set_properties $form_name tel_fluido_vett_term_uscita_gen -value $tel_fluido_vett_term_uscita_gen_edit
	
#	set tel_alimentazione_gen           [string trim [element::get_value $form_name tel_alimentazione_gen]]
#	set tel_alimentazione               [string trim [element::get_value $form_name tel_alimentazione]]
#	set tel_fluido_vett_term_uscita_gen [string trim [element::get_value $form_name tel_fluido_vett_term_uscita_gen]]
#	set tel_alimentazione_altro         [string trim [element::get_value $form_name tel_alimentazione_altro]]
    }
    set tel_fluido_altro                      [string trim [element::get_value $form_name tel_fluido_altro]]
    set tel_potenza_compatibile_dati_prog     [string trim [element::get_value $form_name tel_potenza_compatibile_dati_prog]]
    set tel_stato_coibent_idoneo_scamb        [string trim [element::get_value $form_name tel_stato_coibent_idoneo_scamb]]
    set tel_disp_regolaz_controll_funzionanti [string trim [element::get_value $form_name tel_disp_regolaz_controll_funzionanti]]
#    set tel_assenza_trafil_valv_regolaz       [string trim [element::get_value $form_name tel_assenza_trafil_valv_regolaz]]
    set tel_temp_est                          [string trim [element::get_value $form_name tel_temp_est]]
    set tel_temp_mand_prim                    [string trim [element::get_value $form_name tel_temp_mand_prim]]
    set tel_temp_rit_prim                     [string trim [element::get_value $form_name tel_temp_rit_prim]]
    set tel_potenza_termica                   [string trim [element::get_value $form_name tel_potenza_termica]]
    set tel_portata_fluido_prim               [string trim [element::get_value $form_name tel_portata_fluido_prim]]
    set tel_temp_mand_sec                     [string trim [element::get_value $form_name tel_temp_mand_sec]]
    set tel_temp_rit_sec                      [string trim [element::get_value $form_name tel_temp_rit_sec]]
    set tel_check_coerenza_paramentri         [string trim [element::get_value $form_name tel_check_coerenza_paramentri]]
    set tel_check_perdite_h2o                 [string trim [element::get_value $form_name tel_check_perdite_h2o]]
    set tel_check_install_involucro           [string trim [element::get_value $form_name tel_check_install_involucro]]

    if {$flag_mod_gend == "S"} {
	set costruttore      [element::get_value $form_name costruttore]
	if {$coimtgen(regione) eq "MARCHE"} {#gac03 aggiunta if e suo contenuto
	    set descr_cost      [element::get_value $form_name descr_cost]
	    set flag_clima_invernale_gen [element::get_value $form_name flag_clima_invernale_gen];#gac03
	    set flag_prod_acqua_calda_gen   [element::get_value $form_name flag_prod_acqua_calda_gen];#gac03
	}
...
#    element set_properties $form_name tel_assenza_trafil_valv_regolaz       -value $tel_assenza_trafil_valv_regolaz
	...
	if {$cod_opmanu_new eq ""} {
	    ...
	    #rom04 sostituito $coimtgen(ente) eq "PSA" con $coimtgen(regione) eq "CAMPANIA"
	...
	set sw_data_controllo_ok "f"
		...
		#rom08 tolte condizioni su $coimtgen(ente) eq "PUD" $coimtgen(ente) eq "PGO"  $coimtgen(ente) eq "PTS" e $coimtgen(ente) eq "PPN"
		#rom08 Uso una if unica 
	...
	#rom12 Sostiuitte le condizioni su enti PUD e PGO con una condizione generale su tutta Regione Friuli.
	...
	if {$flag_portafoglio == "T"} {
		...
		if {$data_controllo < "20080801"} {
			...
			if {$data_controllo <= $data_insta_check} {
			    #rom08if {$coimtgen(ente) ne "PUD" &&
			    #rom08$coimtgen(ente) ne "PGO" &&
			    #rom08$coimtgen(ente) ne "PPN" &&
			    #rom08$coimtgen(ente) ne "PTS"} {}#sim44 aggiunto if
			    if {$coimtgen(regione) eq "FRIULI-VENEZIA GIULIA"} {#rom08 aggiunta if ma non contenuto
                                set importo_tariffa "0.00"
                                set tariffa_reg "8"
                            }
			}
	...
	if {$flag_mod_gend == "S"} {
	    ...
	    # serena
	    ns_log notice "SERENA|$flag_mod_gend|$combustibile|"
	    set descr_combustibile [db_string query "select descr_comb from coimcomb where cod_combustibile = :combustibile" -default ""];#--- mis01 estratta descrizione e modificata condizione sotto (origine && $cobustibile=="GASOLIO")
	...
	#rom08 Aggiunta condizione su Regione Friuli
	...
	if {![string equal $riferimento_pag ""] && $tipologia_costo == "BO"} {
		...
		if {$prosegui eq "t" && $potenza_impianto >= 35.00} {
			...
			db_foreach sel_boll_manu "" {
			    if {$matricola_da <= $riferimento_pag_num && $matricola_a >= $riferimento_pag_num} {
				ns_log Notice "coimdimp-r3-gest;$matricola_da <= $riferimento_pag_num && $matricola_a >= $riferimento_pag_num"
				set flag_boll_compreso "t"
			    }
			}
			...
			if {$flag_boll_compreso == "f"} {
			    db_foreach sel_boll_boap "" {
				if {$matr_da <= $riferimento_pag && $matr_a >= $riferimento_pag} {
				    ns_log Notice "coimdimp-r3-gest;$matr_da <= $riferimento_pag && $matr_a >= $riferimento_pag"
				    set flag_boll_compreso "t"
				}
			    }
			}
		...
		set bol_nudo [iter_check_num $bol_nudo 0]
		ns_log Notice "coimdimp-r3-gest;bol_nudo:$bol_nudo"
		set flag_boll_compreso "f"
		db_foreach sel_boll_manu "" {
		    set matricola_da [iter_check_num $matricola_da 0]
		    set matricola_a  [iter_check_num $matricola_a 0]
		...
		set bol_nudo [iter_check_num $bol_nudo 0]
		ns_log Notice "coimdimp-r3-gest;bol_nudo:$riferimento_pag - $bol_nudo"
		set flag_boll_compreso "f"
		db_foreach sel_boll_manu "" {
		    set matricola_da [iter_check_num $matricola_da 0]
		    set matricola_a  [iter_check_num $matricola_a 0]
		...
		# Caso standard
		ns_log Notice "coimdimp-r3-gest;dentro;cod_manutentore:$cod_manutentore;flag_boll_compreso:$flag_boll_compreso"
		db_foreach sel_boll_manu "" {
		    if {$matricola_da <= $riferimento_pag
		    &&  $matricola_a  >= $riferimento_pag
		    } {
			ns_log Notice "coimdimp-r3-gest;matricola_da=$matricola_da<=riferimento_pag:$riferimento_pag;matricola_a:$matricola_a>=riferimento_pag:$riferimento_pag"
			set flag_boll_compreso "t"
	...
	# Quando si modifica un sorgente, va modificato anche l'altro.
	    ...
	    #rom10 Aggiunta condizione 1 == 0
	    if {$potenza_impianto < 100.00 && 1 == 0} {
	    ...
	    if {[string equal $data_scadenza_autocert ""]} {#rom10 Aggiunta if e il suo contenuto
		if {$sw_data_controllo_ok == "t"} {			
		    set data_scadenza_autocert [db_string query "select :data_controllo::date + interval '4 year'" -default ""]
		}
	    } else {#rom10 Aggiunta else e il suo contenuto
		set data_scadenza_autocert [iter_check_date $data_scadenza_autocert]
		if {$data_scadenza_autocert == 0} {
		    element::set_error $form_name data_scadenza_autocert "Inserire correttamente"
		    incr error_num
	    ...
	    #rom08 $coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO" ||
	...
	} elseif {$coimtgen(regione) eq "BASILICATA"} {#rom05 Aggiunta if e suo contenuto

	    #rom06 Modificata la scadenza da 4 a 2 anni per nuova legge Regionale.
	    # Per Impianti alimentati da Teleriscaldamento se la potenza dell impianto
	    # è > 10.00 kw la scadenza e' 4 anni dopo la data del controllo.
	    # Se e' minore allora non è un impianto termico (Sandro dixit).

	    if {$sw_data_controllo_ok == "t"} {

		#rom06set data_scadenza_autocert [db_string query "select :data_controllo::date + interval '4 year'" -default ""]
		set data_scadenza_autocert [db_string query "select :data_controllo::date + interval '2 year'" -default ""];#rom06
            }
...
# san fine 20032017
    ...
    # Inizio teleriscaldamento
    if {$coimtgen(regione) ne "MARCHE"} {
	
	if {$tel_alimentazione eq ""} {
	    element::set_error $form_name tel_alimentazione "Inserire alimentazone"
	    incr error_num    
	}
	if {$tel_alimentazione ne "Z" && $tel_alimentazione_altro ne ""} {
	    element::set_error $form_name tel_alimentazione_altro "Scrivere la nota solo se è stata selezionata la tipologia altro"
	    incr error_num
	}
	if {$tel_alimentazione eq "Z" && $tel_alimentazione_altro eq ""} {
	    element::set_error $form_name tel_alimentazione_altro "E' obbligatorio specificare il campo note se è stata selezionata la tipologia altro"
	    incr error_num
	}
    ...
    if {$tel_pot_obbl eq "t"} {#rom02 aggiunta if

	if {[string equal $tel_temp_est ""] || [string equal $tel_temp_est "0"]} {
	    element::set_error $form_name tel_temp_est "Inserire"
	    incr error_num
	}
	if {[string equal $tel_temp_mand_prim ""] || [string equal $tel_temp_mand_prim "0"]} {
	    element::set_error $form_name tel_temp_mand_prim "Inserire"
	    incr error_num
	}
	if {[string equal $tel_temp_rit_prim ""] || [string equal $tel_temp_rit_prim "0"]} {
	    element::set_error $form_name tel_temp_rit_prim "Inserire"
	    incr error_num
	}
	if {[string equal $tel_potenza_termica ""] || [string equal $tel_potenza_termica "0"]} {
	    element::set_error $form_name tel_potenza_termica "Inserire"
	    incr error_num
	}
	if {[string equal $tel_portata_fluido_prim ""] || [string equal $tel_portata_fluido_prim "0"]} {
	    element::set_error $form_name tel_portata_fluido_prim "Inserire"
	    incr error_num
	}
	if {[string equal $tel_temp_mand_sec ""] || [string equal $tel_temp_mand_sec "0"]} {
	    element::set_error $form_name tel_temp_mand_sec "Inserire"
	    incr error_num
	}
	if {[string equal $tel_temp_rit_sec ""] || [string equal $tel_temp_rit_sec "0"]} {
	    element::set_error $form_name tel_temp_rit_sec "Inserire"
	    incr error_num
	}
    };#rom02

    if {![string equal $tel_temp_est ""]} {
        set tel_temp_est [iter_check_num $tel_temp_est 0]
        if {$tel_temp_est == "Error"} {
            element::set_error $form_name tel_temp_est "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_temp_est] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_temp_est] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_temp_est "Il valore deve essere inferiore a 10.000"
                    incr error_num
            }
        }
    }

    if {![string equal $tel_temp_mand_prim ""]} {
        set tel_temp_mand_prim [iter_check_num $tel_temp_mand_prim 0]
        if {$tel_temp_mand_prim == "Error"} {
            element::set_error $form_name tel_temp_mand_prim "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_temp_mand_prim] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_temp_mand_prim] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_temp_mand_prim "Il valore deve essere inferiore a 10.000"
                    incr error_num
            }
        }
    }
    if {![string equal $tel_temp_rit_prim ""]} {
        set tel_temp_rit_prim [iter_check_num $tel_temp_rit_prim 0]
        if {$tel_temp_rit_prim == "Error"} {
            element::set_error $form_name tel_temp_rit_prim "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_temp_rit_prim] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_temp_rit_prim] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_temp_rit_prim "Il valore deve essere inferiore a 10.000"
                    incr error_num
            }
        }
    }
    if {![string equal $tel_potenza_termica ""]} {
        set tel_potenza_termica [iter_check_num $tel_potenza_termica ]
        if {$tel_potenza_termica == "Error"} {
            element::set_error $form_name tel_potenza_termica "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_potenza_termica] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_potenza_termica] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_potenza_termica "Il valore deve essere inferiore a 10.000"
                    incr error_num
            }
        }
    }
    if {![string equal $tel_portata_fluido_prim ""]} {
        set tel_portata_fluido_prim [iter_check_num $tel_portata_fluido_prim 0]
        if {$tel_portata_fluido_prim == "Error"} {
            element::set_error $form_name tel_portata_fluido_prim "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_portata_fluido_prim] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_portata_fluido_prim] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_portata_fluido_prim "Il valore deve essere inferiore di 10.000"
                    incr error_num
            }
        }
    }
    if {![string equal $tel_temp_mand_sec ""]} {
        set tel_temp_mand_sec [iter_check_num $tel_temp_mand_sec ]
        if {$tel_temp_mand_sec == "Error"} {
            element::set_error $form_name tel_temp_mand_sec "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_temp_mand_sec] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_temp_mand_sec] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_temp_mand_sec "Il valore deve essere inferiore a 10.000"
                    incr error_num
            }
        }
    }
    if {![string equal $tel_temp_rit_sec ""]} {
        set tel_temp_rit_sec [iter_check_num $tel_temp_rit_sec 0]
        if {$tel_temp_rit_sec == "Error"} {
            element::set_error $form_name tel_temp_rit_sec "Questo campo deve essere numerico."
                incr error_num
        } else {
            if {[iter_set_double $tel_temp_rit_sec] >=  [expr pow(10,4)]
                ||  [iter_set_double $tel_temp_rit_sec] <= -[expr pow(10,4)]} {
                    element::set_error $form_name tel_temp_rit_sec "Il valore deve essere inferiore a 10.000"
                    incr error_num
            }
        }
    }
    if {$coimtgen(regione) eq  "MARCHE"} {#rom03 aggiunta if e suo contenuto
	if {$cont_rend eq "S"} {
	    if {$tel_temp_est eq ""} {
		element::set_error $form_name tel_temp_est "Inserire"
		incr error_num
	    }
	    if {$tel_temp_mand_prim eq ""} {
		element::set_error $form_name tel_temp_mand_prim "Inserire"
		incr error_num
	    }
	    if {$tel_temp_rit_prim eq ""} {
		element::set_error $form_name tel_temp_rit_prim "Inserire"
		incr error_num
	    }
	    if {$tel_potenza_termica eq ""} {
		element::set_error $form_name tel_potenza_termica "Inserire"
		incr error_num
	    }
	    if {$tel_portata_fluido_prim eq ""} {
		element::set_error $form_name tel_portata_fluido_prim "Inserire"
		incr error_num
	    }
	    if {$tel_temp_mand_sec eq ""} {
		element::set_error $form_name tel_temp_mand_sec "Inserire"
		incr error_num
	    }
	    if {$tel_temp_rit_sec eq ""} {
		element::set_error $form_name tel_temp_rit_sec "Inserire"
		incr error_num
	    }
	} else {
	    if {$tel_temp_est ne ""
		|| $tel_temp_mand_prim ne ""
		|| $tel_temp_rit_prim ne ""
		|| $tel_potenza_termica ne ""
		|| $tel_portata_fluido_prim ne ""
		|| $tel_temp_mand_sec ne ""
		|| $tel_temp_rit_sec ne ""} {
		element::set_error $form_name cont_rend "Non inserire la parte relativa ai controlli dell'efficienza energetica in quanto il controllo non e&grave; stato effettuato"
		incr error_num
	    }
	}
    }
    # Fine teleriscaldamento
    if {$flag_combo_tipibol eq "T" && $coimtgen(regione) eq "FRIULI-VENEZIA GIULIA"} {#rom09 aggiunta if e suo contenuto
            element::set_value $form_name flag_pagato "S"
	set flag_pagato [string trim [element::get_value $form_name flag_pagato]]
    }
	...
	if {$is_warning_p eq "f"} {#gac02 aggiunto if e suo contenuto
	    element::set_error $form_name msg_rcee "<font color=blue>ATTENZIONE: confermando l'inserimento del REE applicherai anche il relativo segno identificativo.<font>"
	    incr error_num
	}
	set is_warning_p "t"
    } else {
    ...
    if {$error_num > 0} {
	if {$coimtgen(regione) eq "MARCHE"} {#gac02
	    if {$is_warning_p eq "t"} {#gac05 aggiunto if e suo contenuto
		element::set_error $form_name err_rcee "<font color=blue>ATTENZIONE: sono presenti degli errori nella pagina. Cliccando di nuovo su \"Conferma inserimento\", il REE verrà comunque inviato ma sarà sottoposto ad accertamento documentale in via prioritaria.<font>"
	    } else {
		element::set_error $form_name err_rcee "ATTENZIONE: sono presenti degli errori bloccanti nella pagina. Per inviare il REE, occorre correggerli."
	    }
	} else {
	    if {$flag_errore_data_controllo == "f"} {
    ...
    if {$coimtgen(regione) eq "MARCHE"} {
#	set tel_alimentazione $tel_alimentazione_gen
#	set tel_fluido_vett_term_uscita_gen $tel_fluido_vett_term_uscita_gen
    }
    ...
    switch $funzione {
		...
		set cod_dimp $cod_dimp_ins
            ...
            #rom13 Aggiunta condizione per Napoli
	    #rom14 Aggiunta condizione per Rieti
	    #rom15 Aggiunta condizione per Caserta
	    #rom15 Aggiunta condizione per Frosinone
            if {$coimtgen(ente) in [list "PPA" "PNA" "PRI" "PCE" "PFR"] && $costo > 0.00} {#rom07 Aggiunte if, else e il loro contenuto
