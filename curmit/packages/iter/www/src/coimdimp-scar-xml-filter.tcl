ad_page_contract {
    ...
    @param nome_funz identifica l'entrata di menu, serve per le autorizzazioni
                     serve se lista e' uno zoom che permetti aggiungi.
    @cvs-id          coimdimp-scar-xml-filter.tcl
...
set titolo   "Scarico RCEE xml"
switch $funzione {
    I {set button_label "Conferma"
       set page_title   "$titolo "}
    V {set page_title   "$titolo"}
}
...
if {[form is_valid $form_name]} {
  ...
  # Lancio la query di manipolazione dati contenute in dml_sql
    ...
    set link_gest [export_url_vars cod_batc nome_funz nome_funz_caller caller f_cod_manu f_data_ins_iniz f_data_ins_fine f_data_controllo_iniz f_data_controllo_fine]

    switch $funzione {
        I {set return_url   "coimdimp-scar-xml-rcee?funzione=V&$link_gest"}
        V {set return_url   "coimdimp-scar-xml-rcee?funzione=V&$link_gest"}
    }
